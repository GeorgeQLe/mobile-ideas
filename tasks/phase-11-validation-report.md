...
React Native and Expo validation passed for the 19 downstream repositories that currently have package manifests.
...
The following 8 downstream repositories are not validatable as React Native or Expo packages because both `variants/react-native/package.json` and `variants/expo/package.json` are absent on `main`.
...
- Not proven: 16 React Native/Expo package validations for the 8 manifest-missing repos.
