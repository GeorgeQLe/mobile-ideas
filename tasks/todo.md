...
- [ ] Step 8.3: Promote IDs 101-1000 from Draft 1 to implementation-ready public-source V1 (absorbs Phase 7 Step 7.3; scope = 900 specs). Run in category-coherent batches. See archived Phase 7 Step 7.3 detail in `tasks/phases/phase-7.md` for the full operation: exact-URL replacement, verified-vs-inferred discipline, depth expansion, Manual verification blockers per app, and category risk review. Validation: every promoted spec's `Readiness status` line reads `Implementation-ready for a lawful public-source V1 clone as of YYYY-MM-DD.`; no residual `Source discovery -- pending exact URL verification` strings in promoted specs. Progress through 2026-05-05: IDs 101-640 promoted; post-slice audit reports 640 implementation-ready specs and 1,080 remaining placeholder rows across 360 files. Latest slice completed: IDs 621-640 (`Apartment List` through `Wyze`) promoted as a real-estate/home-services plus smart-home category batch. The slice preserved listing/search marketplaces, lead/contact/tour workflows, provider/contractor trust, fair-housing and scam controls, product/order/project workflows, device onboarding, automations, live status simulation, camera/security event handling, shared users, subscription gates, hardware/security blockers, privacy, accessibility, and regional availability blockers. Next concrete slice: promote IDs 641-660 (`Arlo Secure` through `CVS Health`) as a smart-home plus health/medical category batch from `tasks/ideas.md`, replacing source-discovery placeholders with exact first-party URLs and preserving device onboarding, automations, live status simulation, shared users, subscription gates, hardware/security blockers, consent-based onboarding, care/records/reminder flows, HIPAA/consumer-health-data privacy boundaries, accessibility, and regional/provider availability blockers.
