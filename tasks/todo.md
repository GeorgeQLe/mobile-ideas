...
- [~] Step 8.6: Seed downstream private scaffold repos for IDs 201-1000 in serial 20-ID batches via `scripts/seed-downstream-batch.mjs`. **Progress: IDs 201-480 seeded (PRIVATE + non-empty verified per `tasks/repo-seeding.md` Batch evidence sections). Next batch: 481-500.**

  #### Next concrete action: seed batch 481-500

  - Pre-batch: `gh api rate_limit` and record in `tasks/repo-seeding.md`.
  - Run `node scripts/seed-downstream-batch.mjs` for IDs 481-500 with the rolling hourly cap enabled (script enforces ≥30s between repo seeds and ≤20 repos/hour by default, ≤40/hour after two clean batches).
  ...
  - Post-batch: `gh api rate_limit` and record. Append `### Batch 481-500 Seeding Evidence - <ISO timestamp>` section to `tasks/repo-seeding.md` with per-repo URL and root commit SHA.
  - Commit pattern: `feat(seeding): seed private batch 481-500` plus a paired `feat(builds): track downstream build starts 481-500` if the build-tracking workstream continues alongside seeding (see prior history entries).
  - Acceptance: `grep -c "^| \[x\] | [4][89][0-9] " tasks/repo-seeding.md` reflects new completions for the 481-500 range; manifest count for IDs 481-500 = 20 done; no new public repos.
...
- **Phase 8 Step 8.6 progress tracking.** Carried in this todo.md (`Next batch: 481-500`).
