...
- Do not imply affiliation with NBA or its publisher.
...
|---|---|---|---|
| Official product | https://www.nba.com/app | Public product positioning, core surfaces, availability framing, account, catalog/feed, subscription, and app behavior | Verified 2026-05-04 |
| Support/help center | https://support.watch.nba.com/ | Public support taxonomy for account, playback/activity/scoring, billing, devices, permissions, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.nba.com/privacy-policy | Account, profile, media/activity/history, device, advertising, analytics, support, children/profile, location, health, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.nba.com/termsofuse | Service usage, licensed content, subscriptions, user conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/nba-live-games-scores/id335744614 | Canonical iOS listing, category, age rating, privacy label, compatibility, in-app purchases where public, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.nbaimd.gametime.nba2011 | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `Profile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `TeamFollow`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `LeagueFollow`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `EventOrGame`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `ScoreSnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `NewsOrVideoItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NBA-inspired sports and fitness workflows.
