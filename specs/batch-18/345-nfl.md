...
- Do not imply affiliation with NFL or its publisher.
...
|---|---|---|---|
| Official product | https://www.nfl.com/apps/ | Public product positioning, core surfaces, availability framing, account, catalog/feed, subscription, and app behavior | Verified 2026-05-04 |
| Support/help center | https://support.nfl.com/ | Public support taxonomy for account, playback/activity/scoring, billing, devices, permissions, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.nfl.com/legal/privacy/ | Account, profile, media/activity/history, device, advertising, analytics, support, children/profile, location, health, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.nfl.com/legal/terms/ | Service usage, licensed content, subscriptions, user conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/nfl/id389781154 | Canonical iOS listing, category, age rating, privacy label, compatibility, in-app purchases where public, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.gotv.nflgamecenter.us.lite | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `Profile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `TeamFollow`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `LeagueFollow`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `EventOrGame`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `ScoreSnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `NewsOrVideoItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for NFL-inspired sports and fitness workflows.
