# Mercari-Style Clone Spec

> Metadata
> - Inspiration app: Mercari
...
Build an original mobile product inspired by Mercari's public product and policy materials. V1 focuses on consumer marketplace discovery, listing creation, buyer/seller profiles, offers, messaging, payments, shipping labels, returns/disputes, ratings, prohibited-goods moderation, and support. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support consumer marketplace discovery, listing creation, buyer/seller profiles, offers, messaging, payments, shipping labels, returns/disputes, ratings, prohibited-goods moderation, and support with clear retry, recovery, unavailable, region, entitlement, inventory, seller, and provider-aware states.
- Preserve boundaries between buyer, seller, listing, authenticity, payment, payout, shipping, returns, disputes, moderation, advertising, and marketplace data.
...
- Do not imply affiliation with Mercari or its publisher.
...
|---|---|---|---|
| Official product | https://www.mercari.com/ | Public product positioning, shopping, delivery, catalog, marketplace, membership, account, and app behavior | Verified 2026-05-04 |
| Support/help center | https://www.mercari.com/us/help_center/ | Public support taxonomy for account, ordering, payment, pickup, shipping, returns, seller, loyalty, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.mercari.com/us/privacy/ | Account, profile, order, listing, location, device, advertising, analytics, support, seller, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.mercari.com/us/tos/ | Service usage, licensed content, payments, returns, marketplace conduct, seller/buyer limits, prohibited goods, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or seller program | https://www.mercari.com/us/sell/ | Reward, membership, loyalty, seller, service, authenticity, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/mercari-buy-sell-things/id896130944 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.mercariapp.mercari | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- consumer marketplace discovery, listing creation, buyer/seller profiles, offers, messaging, payments, shipping labels, returns/disputes, ratings, prohibited-goods moderation, and support must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
- counterfeit/authenticity review, prohibited goods, seller fraud, buyer fraud, chargebacks, payout holds, tax reporting, listing moderation, harassment in messaging, shipping disputes, and return abuse must be treated as a launch-blocking risk area with owner, mitigation, and acceptance tests.
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `BuyerProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `SellerProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `Listing`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `CatalogTaxonomy`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `ListingMedia`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `SavedSearch`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `MessageThread`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `PaymentIntent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `PayoutInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `Shipment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `AuthenticityReview`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `ReturnCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `DisputeCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `ModerationCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Mercari-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Mercari.
