# OsmAnd-Style Clone Spec

> Metadata
> - Inspiration app: OsmAnd
> - Category: Navigation
...
> - Manual verification blockers: OsmAnd account creation (optional, for cloud sync), OpenStreetMap data processing and offline tile rendering, offline map packaging and download management (worldwide coverage, incremental updates), turn-by-turn offline navigation engine (driving, cycling, walking, skiing routing profiles), offline POI search and address geocoding, topographic map rendering with SRTM contour lines and hillshading, nautical chart and ski map plugin data sourcing, public transit layer data integration, GPX trip recording and track management, OpenStreetMap editing (OSM Live) submission and conflict handling, customizable map rendering engine (styles, overlays, transparency), route planning with waypoints and profile-based routing (fastest, shortest, fuel-efficient), offline Wikipedia article overlay and display, favorites and waypoint import/export (GPX, KML), OsmAnd Cloud backup and cross-device sync infrastructure, plugin architecture and third-party plugin management, OsmAnd+ paid entitlement and Free tier download limits, voice guidance TTS configuration, and OsmAnd account integration.
...
Build an original mobile product inspired by OsmAnd's public navigation workflow. The V1 clone focuses on open-source offline mapping and navigation app based on OpenStreetMap with comprehensive offline maps for worldwide coverage, turn-by-turn navigation for driving, cycling, walking, and skiing with voice guidance, detailed offline POI search and address lookup, topographic maps with contour lines and hillshading (SRTM data), nautical charts and ski maps as specialized plugins, public transit layer in supported cities, trip recording with GPX track logging, OpenStreetMap editing directly from the app (OSM Live), customizable map rendering with multiple styles and overlays, route planning with intermediate waypoints and route profiles, offline Wikipedia articles overlaid on map, favorites and waypoints with import/export, OsmAnd Cloud for backup and cross-device sync, accessibility features including voice navigation, plugin architecture for extensibility, development fund model via OsmAnd+ (paid) and OsmAnd Free (limited downloads).
...
- Preserve the functional job behind OsmAnd while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy OsmAnd branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/osmand-maps-travel-navigate/id934850257 | iOS listing, privacy labels, release notes, offline maps/OSM claims, and support links | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=net.osmand | Android listing, data safety, offline navigation/OpenStreetMap claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official product site | https://osmand.net/ | OsmAnd offline maps, navigation, OpenStreetMap, and product positioning | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official help | https://osmand.net/docs/intro | OsmAnd app help, documentation, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Privacy policy | https://osmand.net/docs/legal/privacy-policy | OsmAnd data collection, location data, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Terms | https://osmand.net/docs/legal/terms-of-use | OsmAnd services, usage terms, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: OsmAnd account creation (optional, for cloud sync), OpenStreetMap data processing and offline tile rendering, offline map packaging and download management (worldwide coverage, incremental updates), turn-by-turn offline navigation engine (driving, cycling, walking, skiing routing profiles), offline POI search and address geocoding, topographic map rendering with SRTM contour lines and hillshading, nautical chart and ski map plugin data sourcing, public transit layer data integration, GPX trip recording and track management, OpenStreetMap editing (OSM Live) submission and conflict handling, customizable map rendering engine (styles, overlays, transparency), route planning with waypoints and profile-based routing (fastest, shortest, fuel-efficient), offline Wikipedia article overlay and display, favorites and waypoint import/export (GPX, KML), OsmAnd Cloud backup and cross-device sync infrastructure, plugin architecture and third-party plugin management, OsmAnd+ paid entitlement and Free tier download limits, voice guidance TTS configuration, and OsmAnd account integration.

## Core User Journeys

- User creates or restores a OsmAnd-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from OsmAnd.
...
- Manual verification tests for OsmAnd account creation (optional, for cloud sync), OpenStreetMap data processing and offline tile rendering, offline map packaging and download management (worldwide coverage, incremental updates), turn-by-turn offline navigation engine (driving, cycling, walking, skiing routing profiles), offline POI search and address geocoding, topographic map rendering with SRTM contour lines and hillshading, nautical chart and ski map plugin data sourcing, public transit layer data integration, GPX trip recording and track management, OpenStreetMap editing (OSM Live) submission and conflict handling, customizable map rendering engine (styles, overlays, transparency), route planning with waypoints and profile-based routing (fastest, shortest, fuel-efficient), offline Wikipedia article overlay and display, favorites and waypoint import/export (GPX, KML), OsmAnd Cloud backup and cross-device sync infrastructure, plugin architecture and third-party plugin management, OsmAnd+ paid entitlement and Free tier download limits, voice guidance TTS configuration, and OsmAnd account integration.
...
- Phase 4: Add app-specific safety controls for open-source offline mapping and navigation app based on OpenStreetMap with comprehensive offline maps for worldwide coverage, turn-by-turn navigation for driving, cycling, walking, and skiing with voice guidance, detailed offline POI search and address lookup, topographic maps with contour lines and hillshading (SRTM data), nautical charts and ski maps as specialized plugins, public transit layer in supported cities, trip recording with GPX track logging, OpenStreetMap editing directly from the app (OSM Live), customizable map rendering with multiple styles and overlays, route planning with intermediate waypoints and route profiles, offline Wikipedia articles overlaid on map, favorites and waypoints with import/export, OsmAnd Cloud for backup and cross-device sync, accessibility features including voice navigation, plugin architecture for extensibility, development fund model via OsmAnd+ (paid) and OsmAnd Free (limited downloads).
...
- Confirm legal/privacy retention behavior for OsmAnd-style sensitive records and update API contracts before downstream implementation.
