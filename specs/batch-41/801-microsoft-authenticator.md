# Microsoft Authenticator-Style Clone Spec

> Metadata
> - Inspiration app: Microsoft Authenticator
...
> - Manual verification blockers: Microsoft account and Azure AD/Entra ID tenant policy, push notification approval delivery and timing, TOTP generation accuracy and time sync, passwordless sign-in registration and challenge flow, certificate-based authentication enrollment, biometric lock security model, cloud backup encryption and cross-platform recovery, autofill integration (iOS AutoFill, Android Autofill API), Verified ID presentation flow, Conditional Access compliance reporting, enterprise MDM/MAM policy enforcement, multi-account switching, and tenant-specific feature availability.
...
Build an original mobile product inspired by Microsoft Authenticator's public cloud files and identity workflow. The V1 clone focuses on multi-factor authentication with push approval, TOTP code generation, passwordless sign-in, Microsoft account and Azure AD/Entra ID work/school account management, certificate-based authentication, app lock with biometrics, cloud backup and recovery, autofill for passwords/addresses/payments, verified IDs, and enterprise Conditional Access compliance.
...
- Deliver a mobile-first cloud files and identity experience with onboarding, dashboard, primary workflow, reminders or notifications, settings, support, and recovery flows.
- Preserve the functional job behind Microsoft Authenticator while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Microsoft Authenticator branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/microsoft-authenticator/id983156458 | iOS listing, privacy labels, release notes, MFA/passwordless claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.azure.authenticator | Android listing, data safety, MFA/autofill claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://www.microsoft.com/en-us/security/mobile-authenticator-app | Microsoft Authenticator MFA, passwordless, autofill, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://support.microsoft.com/en-us/account-billing/download-and-install-the-microsoft-authenticator-app-351498fc-850a-45da-b7b6-27e523b8702a | Authenticator setup, MFA, backup, recovery, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy statement | https://privacy.microsoft.com/privacystatement | Microsoft account, tenant, cloud, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.microsoft.com/servicesagreement | Microsoft services agreement, account, content, subscriptions, and service boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Microsoft account and Azure AD/Entra ID tenant policy, push notification approval delivery and timing, TOTP generation accuracy and time sync, passwordless sign-in registration and challenge flow, certificate-based authentication enrollment, biometric lock security model, cloud backup encryption and cross-platform recovery, autofill integration (iOS AutoFill, Android Autofill API), Verified ID presentation flow, Conditional Access compliance reporting, enterprise MDM/MAM policy enforcement, multi-account switching, and tenant-specific feature availability.

## Core User Journeys

- User creates or restores a Microsoft Authenticator-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
|---|---|---|---|---|
| Welcome/Auth | Account creation, organization/tenant enrollment, SSO, biometric setup, and consent entry | email, SSO, biometric, QR | new, returning, enrolled, locked | SSO failed, tenant unavailable, biometric denied, account blocked |
| Accounts List | Enrolled accounts/tokens, organization grouping, push pending indicators, and account management | tap, search, add, remove | empty, loading, current, push-pending | account expired, token removed, org deactivated, sync failed |
| Code/Push Detail | TOTP/HOTP code display with countdown, push approval prompt, number matching, and copy action | approve, deny, copy, refresh | code-active, push-pending, approved, denied | code expired, push timeout, number mismatch, network unavailable |
| Add Account | QR code scanning, manual key entry, push enrollment, and third-party account setup | scan, manual-entry, enroll, confirm | scanning, entering, enrolling, confirmed | QR invalid, key format error, enrollment failed, camera denied |
| Security/Device | Biometric lock, device trust status, compliance signals, backup status, and app lock settings | toggle, verify, configure, backup | trusted, untrusted, compliant, non-compliant | biometric unavailable, device rooted, compliance failed, backup error |
| Settings/Privacy | Account management, backup/restore, notification preferences, app lock, privacy, support, and deletion | forms, toggle, restore, request | loaded, editing, restoring | backup corrupt, restore failed, active enterprise policy |
...
- Treat authentication secrets, TOTP/HOTP seeds, push approval tokens, encryption keys, biometric templates, device trust signals, cloud backup encryption, enterprise tenant policies, account recovery credentials, organization enrollment data, compliance attestations, and session tokens as launch-blocking review areas with owners, mitigations, and acceptance tests before implementation.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Microsoft Authenticator.
...
- Manual verification tests for Microsoft account and Azure AD/Entra ID tenant policy, push notification approval delivery and timing, TOTP generation accuracy and time sync, passwordless sign-in registration and challenge flow, certificate-based authentication enrollment, biometric lock security model, cloud backup encryption and cross-platform recovery, autofill integration (iOS AutoFill, Android Autofill API), Verified ID presentation flow, Conditional Access compliance reporting, enterprise MDM/MAM policy enforcement, multi-account switching, and tenant-specific feature availability.
...
- Phase 4: Add app-specific safety controls for multi-factor authentication with push approval, TOTP code generation, passwordless sign-in, Microsoft account and Azure AD/Entra ID work/school account management, certificate-based authentication, app lock with biometrics, cloud backup and recovery, autofill for passwords/addresses/payments, verified IDs, and enterprise Conditional Access compliance.
...
- Confirm legal/privacy retention behavior for Microsoft Authenticator-style sensitive records and update API contracts before downstream implementation.
