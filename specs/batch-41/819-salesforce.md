# Salesforce-Style Clone Spec

> Metadata
> - Inspiration app: Salesforce
...
> - Manual verification blockers: Salesforce org and user license policy, org authentication (SSO/OAuth/SAML), lead/contact/opportunity/account CRUD and validation rules, pipeline stage and forecast behavior, activity logging and task assignment, Chatter feed permissions and @mention delivery, dashboard and report rendering and drill-down, Einstein AI model availability per license, approval workflow routing and delegation, offline sync scope and conflict resolution, push notification delivery for record changes/task assignments/Chatter mentions, multi-org switching and sandbox access, admin-configured page layouts and record types, managed package and AppExchange integrations, Salesforce Shield encryption, and enterprise/compliance certifications.
...
Build an original mobile product inspired by Salesforce's public enterprise operations workflow. The V1 clone focuses on mobile CRM with lead and contact management, opportunity pipeline and deal tracking, account hierarchies, activity timeline, task and event management, Chatter (enterprise social feed), dashboards and reports, Einstein AI insights, approval workflows, offline data access, push notifications for deals/tasks/mentions, Salesforce org and multi-org switching, and enterprise license entitlements.
...
- Preserve the functional job behind Salesforce while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Salesforce branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/salesforce/id404249815 | iOS listing, privacy labels, release notes, CRM/mobile claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.salesforce.chatter | Android listing, data safety, CRM/offline/Einstein claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://www.salesforce.com/solutions/mobile/overview/ | Salesforce Mobile features, offline, Einstein, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://help.salesforce.com/s/articleView?id=sf.salesforce_app.htm | Salesforce Mobile setup, navigation, offline, notifications, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://www.salesforce.com/company/privacy/ | Salesforce account, org data, analytics, AI, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.salesforce.com/company/legal/agreements/ | Salesforce services, subscriptions, enterprise, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Salesforce org and user license policy, org authentication (SSO/OAuth/SAML), lead/contact/opportunity/account CRUD and validation rules, pipeline stage and forecast behavior, activity logging and task assignment, Chatter feed permissions and @mention delivery, dashboard and report rendering and drill-down, Einstein AI model availability per license, approval workflow routing and delegation, offline sync scope and conflict resolution, push notification delivery for record changes/task assignments/Chatter mentions, multi-org switching and sandbox access, admin-configured page layouts and record types, managed package and AppExchange integrations, Salesforce Shield encryption, and enterprise/compliance certifications.

## Core User Journeys

- User creates or restores a Salesforce-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
|---|---|---|---|---|
| Welcome/Auth | Account login, org/portal selection, SSO/SAML, biometric, and permission primers | email, SSO, org-select, biometric | new, returning, multi-org, locked | SSO failed, org unavailable, license expired, auth timeout |
| Home/Feed | Activity feed, recent records, tasks due, notifications, and quick-create actions | tap, search, create, filter | empty, loading, current, offline | sync failed, feed stale, offline cache expired, permission denied |
| Records/Pipeline | Contacts, companies, deals/opportunities, pipeline stages, list views, and search | tap, search, filter, sort, create | empty, loading, filtered, offline | record locked, field validation failed, pipeline stage restricted, sync conflict |
| Record Detail | Contact/deal/account detail, activity timeline, related records, tasks, notes, and edit actions | edit, log, call, email, task | viewing, editing, logging, related | field required, validation error, record deleted, permission denied |
| Activity/Tasks | Task list, activity logging, calls, emails, meetings, notes, and calendar integration | create, complete, log, schedule | pending, completed, overdue, syncing | calendar auth expired, call failed, email bounce, task reassigned |
| Settings/Privacy | Account, org/portal, notifications, offline sync, integrations, privacy, support, and data export | forms, toggle, sync, request | loaded, editing, syncing | org policy override, integration expired, offline limit reached |
...
- Treat CRM contacts and companies, deal pipeline and revenue data, activity logs and call recordings, email tracking data, meeting schedules, chat conversations, document access logs, quote and pricing data, ticket and service records, marketing engagement data, custom fields and properties, approval workflow state, org/tenant configuration, user permissions and roles, and integration credentials as launch-blocking review areas with owners, mitigations, and acceptance tests before implementation.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Salesforce.
...
- Manual verification tests for Salesforce org and user license policy, org authentication (SSO/OAuth/SAML), lead/contact/opportunity/account CRUD and validation rules, pipeline stage and forecast behavior, activity logging and task assignment, Chatter feed permissions and @mention delivery, dashboard and report rendering and drill-down, Einstein AI model availability per license, approval workflow routing and delegation, offline sync scope and conflict resolution, push notification delivery for record changes/task assignments/Chatter mentions, multi-org switching and sandbox access, admin-configured page layouts and record types, managed package and AppExchange integrations, Salesforce Shield encryption, and enterprise/compliance certifications.
...
- Phase 4: Add app-specific safety controls for mobile CRM with lead and contact management, opportunity pipeline and deal tracking, account hierarchies, activity timeline, task and event management, Chatter (enterprise social feed), dashboards and reports, Einstein AI insights, approval workflows, offline data access, push notifications for deals/tasks/mentions, Salesforce org and multi-org switching, and enterprise license entitlements.
...
- Confirm legal/privacy retention behavior for Salesforce-style sensitive records and update API contracts before downstream implementation.
