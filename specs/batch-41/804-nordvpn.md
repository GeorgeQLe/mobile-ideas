# NordVPN-Style Clone Spec

> Metadata
> - Inspiration app: NordVPN
...
> - Manual verification blockers: account and subscription policy, VPN tunnel establishment and protocol negotiation, server selection and load balancing, kill switch enforcement on iOS/Android, split tunneling app/domain configuration, Threat Protection filtering accuracy, Meshnet peer-to-peer routing, Dark Web Monitor credential sourcing, auto-connect network detection, DNS leak prevention, NordLynx (WireGuard-based) performance, subscription restore, and regional server availability.
...
Build an original mobile product inspired by NordVPN's public security and vpn workflow. The V1 clone focuses on VPN with one-tap connect, server selection by country/specialty (P2P, Double VPN, Onion over VPN), kill switch, split tunneling, Threat Protection (ad/tracker/malware blocking), Meshnet (private encrypted networking), Dark Web Monitor, auto-connect on untrusted networks, custom DNS, protocol selection (NordLynx/OpenVPN/IKEv2), and subscription plan entitlements.
...
- Deliver a mobile-first security and vpn experience with onboarding, dashboard, primary workflow, reminders or notifications, settings, support, and recovery flows.
- Preserve the functional job behind NordVPN while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy NordVPN branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/nordvpn-vpn-fast-secure/id905953485 | iOS listing, privacy labels, release notes, VPN/Threat Protection claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.nordvpn.android | Android listing, data safety, VPN/Meshnet claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://nordvpn.com/ | NordVPN features, servers, Threat Protection, Meshnet, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://support.nordvpn.com/ | NordVPN setup, protocols, kill switch, split tunneling, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://nordvpn.com/privacy-policy/ | Account, VPN logs, analytics, Threat Protection, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://nordvpn.com/terms-of-service/ | Service, subscriptions, VPN usage, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: account and subscription policy, VPN tunnel establishment and protocol negotiation, server selection and load balancing, kill switch enforcement on iOS/Android, split tunneling app/domain configuration, Threat Protection filtering accuracy, Meshnet peer-to-peer routing, Dark Web Monitor credential sourcing, auto-connect network detection, DNS leak prevention, NordLynx (WireGuard-based) performance, subscription restore, and regional server availability.

## Core User Journeys

- User creates or restores a NordVPN-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
|---|---|---|---|---|
| Welcome/Auth | Account creation, subscription activation, permission primers (VPN configuration), and consent entry | email, code, permission, restore | new, returning, trial, expired | auth failed, subscription invalid, VPN permission denied |
| Connect/Home | One-tap connect/disconnect, connection status, selected server/location, data usage, and quick settings | connect, disconnect, select, configure | disconnected, connecting, connected, reconnecting | tunnel failed, server unreachable, kill switch active, network changed |
| Server Selection | Country/city list, specialty servers, server load indicators, favorites, and search | tap, search, favorite, filter | loading, loaded, filtered, empty-results | server full, region unavailable, specialty not available on plan |
| Features/Protection | Kill switch, split tunneling, ad/tracker blocking, DNS settings, protocol selection, and auto-connect rules | toggle, configure, select, add-rule | enabled, disabled, configuring, blocked | feature not on plan, split tunnel conflict, DNS leak detected, protocol unavailable |
| Account/Subscription | Account details, subscription status, plan features, upgrade/downgrade, and device management | view, upgrade, manage, restore | active, trial, expired, canceled | payment failed, restore failed, device limit reached, plan unavailable |
| Settings/Privacy | Protocol, auto-connect, notifications, appearance, privacy, support, terms, and data deletion | forms, toggle, links, request | loaded, editing, requested | active VPN session during change, enterprise policy override |
...
- Treat VPN tunnel credentials, connection metadata, server selection history, DNS queries, split tunneling rules, network trust decisions, kill switch state, ad/tracker blocking rules, dark web monitoring credentials, subscription entitlements, device identifiers, auto-connect network fingerprints, and traffic analysis resistance as launch-blocking review areas with owners, mitigations, and acceptance tests before implementation.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from NordVPN.
...
- Manual verification tests for account and subscription policy, VPN tunnel establishment and protocol negotiation, server selection and load balancing, kill switch enforcement on iOS/Android, split tunneling app/domain configuration, Threat Protection filtering accuracy, Meshnet peer-to-peer routing, Dark Web Monitor credential sourcing, auto-connect network detection, DNS leak prevention, NordLynx (WireGuard-based) performance, subscription restore, and regional server availability.
...
- Phase 4: Add app-specific safety controls for VPN with one-tap connect, server selection by country/specialty (P2P, Double VPN, Onion over VPN), kill switch, split tunneling, Threat Protection (ad/tracker/malware blocking), Meshnet (private encrypted networking), Dark Web Monitor, auto-connect on untrusted networks, custom DNS, protocol selection (NordLynx/OpenVPN/IKEv2), and subscription plan entitlements.
...
- Confirm legal/privacy retention behavior for NordVPN-style sensitive records and update API contracts before downstream implementation.
