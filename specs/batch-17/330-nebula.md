# Nebula-Style Clone Spec

> Metadata
> - Inspiration app: Nebula
...
Build an original mobile streaming video product inspired by Nebula's public product and policy materials. V1 focuses on creator-owned streaming, subscriptions, classes/courses where available, creator channels, downloads, comments/community surfaces, and bundle entitlements. The clone must use original branding, original UI copy, original sample media, licensed providers/assets, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support creator-owned streaming, subscriptions, classes/courses where available, creator channels, downloads, comments/community surfaces, and bundle entitlements with clear retry, recovery, unavailable, region, and rights-aware states.
...
- Do not imply affiliation with Nebula or its publisher.
...
|---|---|---|---|
| Official product | https://nebula.tv/ | Public product positioning, core surfaces, availability framing, account, catalog/feed, subscription, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.nebula.tv/ | Public support taxonomy for account, playback/reading, billing, devices, downloads, provider auth, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://nebula.tv/privacy | Account, profile, media/history, device, advertising, analytics, support, children/profile, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://nebula.tv/terms | Service usage, licensed content, subscriptions, user conduct, provider limits, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/nebula-videos-and-podcasts/id1446173853 | Canonical iOS listing, category, age rating, privacy label, compatibility, in-app purchases where public, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=tv.standard.nebula | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- creator content licensing, subscription/bundle ownership, downloads, comments/community moderation, creator analytics, and off-platform account linking must be treated as a launch-blocking risk area with owner, mitigation, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth | Defines the welcome / auth workflow for Nebula-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
...
| Creator Channel | Defines the creator channel workflow for Nebula-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
| Class / Course | Defines the class / course workflow for Nebula-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `Profile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `CatalogOrFeedItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `CreatorOrPublisher`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `EpisodeOrTitle`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `PlaybackSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `QueueOrWatchlist`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `Download`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `ProviderJob`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `AdOrSponsorshipSlot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `RightsOrSafetyReport`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Nebula-inspired streaming video workflows.
