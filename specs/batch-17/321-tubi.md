# Tubi-Style Clone Spec

> Metadata
> - Inspiration app: Tubi
...
Build an original mobile streaming video product inspired by Tubi's public product and policy materials. V1 focuses on free ad-supported movies and TV, live channels, account watchlists, resume state, parental/rating controls, ads, and region/catalog availability. The clone must use original branding, original UI copy, original sample media, licensed providers/assets, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support free ad-supported movies and TV, live channels, account watchlists, resume state, parental/rating controls, ads, and region/catalog availability with clear retry, recovery, unavailable, region, and rights-aware states.
...
- Do not imply affiliation with Tubi or its publisher.
...
|---|---|---|---|
| Official product | https://tubitv.com/ | Public product positioning, core surfaces, availability framing, account, catalog/feed, subscription, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.tubitv.com/ | Public support taxonomy for account, playback/reading, billing, devices, downloads, provider auth, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://corporate.tubitv.com/privacy-policy/ | Account, profile, media/history, device, advertising, analytics, support, children/profile, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://corporate.tubitv.com/terms-of-use/ | Service usage, licensed content, subscriptions, user conduct, provider limits, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/tubi-free-movies-live-tv/id886445756 | Canonical iOS listing, category, age rating, privacy label, compatibility, in-app purchases where public, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.tubitv | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- FAST streaming, licensed catalog windows, ad-tech identifiers, profile maturity gates, local-news rights, and accountless viewing privacy must be treated as a launch-blocking risk area with owner, mitigation, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth | Defines the welcome / auth workflow for Tubi-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
...
| Live Channels | Defines the live channels workflow for Tubi-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
| Ad Break / Sponsored Slot | Defines the ad break / sponsored slot workflow for Tubi-inspired streaming video behavior | taps, forms, search, filters, deep links, media controls, permissions | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, rights unavailable, provider failed, region unavailable |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `Profile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `CatalogOrFeedItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `CreatorOrPublisher`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `EpisodeOrTitle`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `PlaybackSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `QueueOrWatchlist`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `Download`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `ProviderJob`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `AdOrSponsorshipSlot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `RightsOrSafetyReport`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Tubi-inspired streaming video workflows.
