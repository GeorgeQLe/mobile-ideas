# Thrive Market-Style Clone Spec

> Metadata
> - Inspiration app: Thrive Market
...
Build an original mobile product inspired by Thrive Market's public product and policy materials. V1 focuses on paid membership, member-only pricing, diet/lifestyle filters, autoship reminders, shipping thresholds, gifts/deals, sponsored membership claims, and cancellation/refund handling. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support paid membership, member-only pricing, diet/lifestyle filters, autoship reminders, shipping thresholds, gifts/deals, sponsored membership claims, and cancellation/refund handling with clear retry, recovery, unavailable, region, entitlement, and provider-aware states.
...
- Do not imply affiliation with Thrive Market or its publisher.
...
|---|---|---|---|
| Official product | https://thrivemarket.com/ | Public product positioning, catalog/menu or retail surfaces, ordering/shopping, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.thrivemarket.com/ | Public support taxonomy for account, order/activity, billing/payment, store/provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://thrivemarket.com/privacy | Account, profile, order/purchase, location, device, advertising, analytics, support, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://thrivemarket.com/terms | Service usage, licensed content, payments, user conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards or membership terms | https://thrivemarket.com/membership | Reward, membership, loyalty, offer, stored-value, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/thrive-market/id1077909668 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.thrivemarket.app | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- paid membership, member-only pricing, diet/lifestyle filters, autoship reminders, shipping thresholds, gifts/deals, sponsored membership claims, and cancellation/refund handling must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for Thrive Market-inspired behavior | taps, forms, search, filters, deep links, location, payment, store/merchant selection | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, store/merchant closed, item unavailable, payment failed |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `CustomerProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `StoreLocation`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `Catalog`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `CatalogItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `InventorySnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `Cart`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `ShoppingList`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `Order`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `PaymentInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `RewardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `Fulfillment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `SubstitutionPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `RefundCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Thrive Market-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Thrive Market.
