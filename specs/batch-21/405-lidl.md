# Lidl-Style Clone Spec

> Metadata
> - Inspiration app: Lidl
...
Build an original mobile product inspired by Lidl's public product and policy materials. V1 focuses on Lidl Plus account, coupons, rewards, digital receipts, weekly offers, store selection, shopping list, promotion freshness, and cross-region feature availability. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support Lidl Plus account, coupons, rewards, digital receipts, weekly offers, store selection, shopping list, promotion freshness, and cross-region feature availability with clear retry, recovery, unavailable, region, entitlement, and provider-aware states.
...
- Do not imply affiliation with Lidl or its publisher.
...
|---|---|---|---|
| Official product | https://www.lidl.com/ | Public product positioning, catalog/menu or retail surfaces, ordering/shopping, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.lidl.com/ | Public support taxonomy for account, order/activity, billing/payment, store/provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.lidl.com/privacy-policy | Account, profile, order/purchase, location, device, advertising, analytics, support, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.lidl.com/terms-of-use | Service usage, licensed content, payments, user conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards or membership terms | https://www.lidl.com/lidl-plus | Reward, membership, loyalty, offer, stored-value, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/lidl-us/id1220932787 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.lidl.eci.lidlplus | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- Lidl Plus account, coupons, rewards, digital receipts, weekly offers, store selection, shopping list, promotion freshness, and cross-region feature availability must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for Lidl-inspired behavior | taps, forms, search, filters, deep links, location, payment, store/merchant selection | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, store/merchant closed, item unavailable, payment failed |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `CustomerProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `StoreLocation`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `Catalog`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `CatalogItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `InventorySnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `Cart`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `ShoppingList`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `Order`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `PaymentInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `RewardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `Fulfillment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `SubstitutionPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `RefundCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Lidl-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Lidl.
