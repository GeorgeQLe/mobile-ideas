# Sainsbury's-Style Clone Spec

> Metadata
> - Inspiration app: Sainsbury's
...
Build an original mobile product inspired by Sainsbury's's public product and policy materials. V1 focuses on registered grocery account, Nectar linking, product browse/search, favorites, order creation/editing, delivery or Click & Collect slots, offers, and UK serviceability. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support registered grocery account, Nectar linking, product browse/search, favorites, order creation/editing, delivery or Click & Collect slots, offers, and UK serviceability with clear retry, recovery, unavailable, region, entitlement, and provider-aware states.
...
- Do not imply affiliation with Sainsbury's or its publisher.
...
|---|---|---|---|
| Official product | https://www.sainsburys.co.uk/ | Public product positioning, catalog/menu or retail surfaces, ordering/shopping, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.sainsburys.co.uk/ | Public support taxonomy for account, order/activity, billing/payment, store/provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://privacy-hub.sainsburys.co.uk/privacy-policy/ | Account, profile, order/purchase, location, device, advertising, analytics, support, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://help.sainsburys.co.uk/help/terms-and-conditions | Service usage, licensed content, payments, user conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards or membership terms | https://www.sainsburys.co.uk/gol-ui/nectar | Reward, membership, loyalty, offer, stored-value, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/sainsburys-groceries/id1086056964 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.sainsburys.gol | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- registered grocery account, Nectar linking, product browse/search, favorites, order creation/editing, delivery or Click & Collect slots, offers, and UK serviceability must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for Sainsbury's-inspired behavior | taps, forms, search, filters, deep links, location, payment, store/merchant selection | empty, loading, loaded, signed-out, offline, entitlement-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, store/merchant closed, item unavailable, payment failed |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `CustomerProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `StoreLocation`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `Catalog`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `CatalogItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `InventorySnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `Cart`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `ShoppingList`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `Order`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `PaymentInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `RewardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `Fulfillment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `SubstitutionPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `RefundCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Sainsbury's-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Sainsbury's.
