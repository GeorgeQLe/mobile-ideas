# FlightAware-Style Clone Spec

> Metadata
> - Inspiration app: FlightAware
...
> - Manual verification blockers: flight status freshness, alert delivery, push notification copy, airport delay data, account watchlists, premium/ad states, location prompts, and regional flight-data coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by FlightAware's public user-facing workflow. The clone focus is flight status tracking, airport and aircraft search, alerts, live maps, delay information, account watchlists, premium/ad states, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy FlightAware branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/flightaware-flight-tracker/id316793974 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.flightaware.android.liveFlightTracker | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://flightaware.com/ | Public product orientation for flight status tracking, airport and aircraft search, alerts, live maps, delay information, account watchlists, premium/ad states, and location privacy controls | Public source verified |
| Help/support center | https://support.flightaware.com/ | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://flightaware.com/about/privacy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://flightaware.com/about/termsofuse/ | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: flight status freshness, alert delivery, push notification copy, airport delay data, account watchlists, premium/ad states, location prompts, and regional flight-data coverage.
...
| Flight Status | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Live Map | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Aircraft Detail | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Watchlist | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Delay Board | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `Flight`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `Aircraft`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `Airport`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `FlightPosition`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `DelayStatus`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `FlightAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `Watchlist`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `ProviderAttribution`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FlightAware-style workflows.
...
- Manual verification checklist tests for flight status freshness, alert delivery, push notification copy, airport delay data, account watchlists, premium/ad states, location prompts, and regional flight-data coverage.
...
- Phase 7: Conduct lawful hands-on verification for flight status freshness, alert delivery, push notification copy, airport delay data, account watchlists, premium/ad states, location prompts, and regional flight-data coverage, then update this spec with evidence before any one-for-one parity claim.
