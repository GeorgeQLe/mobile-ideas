# Windy-Style Clone Spec

> Metadata
> - Inspiration app: Windy
...
> - Manual verification blockers: forecast model switching, premium forecast intervals, weather alert creation, widgets, radar/satellite playback, route planner behavior, location prompts, push alerts, and regional forecast-layer availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Windy's public user-facing workflow. The clone focus is interactive weather maps, forecast model comparison, radar and satellite layers, favorites, alerts, widgets, route-oriented planning, premium forecasts, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Windy branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/windy-com/id1161387262 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.windyty.android | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.windy.com/ | Public product orientation for interactive weather maps, forecast model comparison, radar and satellite layers, favorites, alerts, widgets, route-oriented planning, premium forecasts, and location privacy controls | Public source verified |
| Help/support center | https://community.windy.com/ | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://www.windy.com/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.windy.com/terms-and-conditions | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: forecast model switching, premium forecast intervals, weather alert creation, widgets, radar/satellite playback, route planner behavior, location prompts, push alerts, and regional forecast-layer availability.
...
|---|---|---|---|---|
| Weather Map | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Meteogram | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Radar/Satellite | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Favorites | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Route Planner | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ForecastLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `WeatherLayer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `ForecastModel`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `Meteogram`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `WeatherAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `FavoritePlace`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `RouteForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `MapViewport`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
- `WeatherStation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Windy-style workflows.
...
- Manual verification checklist tests for forecast model switching, premium forecast intervals, weather alert creation, widgets, radar/satellite playback, route planner behavior, location prompts, push alerts, and regional forecast-layer availability.
...
- Phase 7: Conduct lawful hands-on verification for forecast model switching, premium forecast intervals, weather alert creation, widgets, radar/satellite playback, route planner behavior, location prompts, push alerts, and regional forecast-layer availability, then update this spec with evidence before any one-for-one parity claim.
