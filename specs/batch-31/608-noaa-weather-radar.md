# NOAA Weather Radar-Style Clone Spec

> Metadata
> - Inspiration app: NOAA Weather Radar
...
> - Manual verification blockers: radar playback, alert delivery, hurricane tracker, premium/ad removal, widgets, NOAA data attribution, location prompts, push alerts, and regional radar coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by NOAA Weather Radar's public user-facing workflow. The clone focus is NOAA-style radar maps, precipitation forecasts, severe alerts, hurricane tracking, saved locations, widgets, premium/ad states, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy NOAA Weather Radar branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/clime-noaa-weather-radar-live/id749133753 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.apalon.weatherradar.free | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://climeradar.com/ | Public product orientation for NOAA-style radar maps, precipitation forecasts, severe alerts, hurricane tracking, saved locations, widgets, premium/ad states, and location privacy controls | Public source verified |
| Help/support center | https://support.climeradar.com/hc/en-us | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://climeradar.com/privacy-policy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://climeradar.com/terms-of-use | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: radar playback, alert delivery, hurricane tracker, premium/ad removal, widgets, NOAA data attribution, location prompts, push alerts, and regional radar coverage.
...
| Layer Picker | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Forecast Detail | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Precipitation Timeline | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Hurricane Tracker | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ForecastLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `RadarFrame`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `WeatherLayer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `PrecipitationForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `SevereAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `StormTrack`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `SavedLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `WidgetConfig`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
- `DataProviderAttribution`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for NOAA Weather Radar-style workflows.
...
- Manual verification checklist tests for radar playback, alert delivery, hurricane tracker, premium/ad removal, widgets, NOAA data attribution, location prompts, push alerts, and regional radar coverage.
...
- Phase 7: Conduct lawful hands-on verification for radar playback, alert delivery, hurricane tracker, premium/ad removal, widgets, NOAA data attribution, location prompts, push alerts, and regional radar coverage, then update this spec with evidence before any one-for-one parity claim.
