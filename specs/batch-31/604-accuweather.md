# AccuWeather-Style Clone Spec

> Metadata
> - Inspiration app: AccuWeather
...
> - Manual verification blockers: MinuteCast behavior, severe alert timing, radar playback, lifestyle indices, premium/ad states, widgets, location prompts, push alerts, and regional forecast coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by AccuWeather's public user-facing workflow. The clone focus is minute-by-minute forecasts, local conditions, severe alerts, radar maps, lifestyle forecasts, saved locations, widgets, premium/ad states, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy AccuWeather branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/accuweather-weather-forecast/id300048137 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.accuweather.android | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.accuweather.com/ | Public product orientation for minute-by-minute forecasts, local conditions, severe alerts, radar maps, lifestyle forecasts, saved locations, widgets, premium/ad states, and location privacy controls | Public source verified |
| Help/support center | https://support.accuweather.com/hc/en-us | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://www.accuweather.com/en/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.accuweather.com/en/legal | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: MinuteCast behavior, severe alert timing, radar playback, lifestyle indices, premium/ad states, widgets, location prompts, push alerts, and regional forecast coverage.
...
|---|---|---|---|---|
| Current Conditions | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Minute Forecast | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Radar Map | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Lifestyle Indices | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ForecastLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `MinuteForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `HourlyForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `DailyForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `RadarFrame`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `SevereAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `LifestyleIndex`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `SavedLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `WidgetConfig`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for AccuWeather-style workflows.
...
- Manual verification checklist tests for MinuteCast behavior, severe alert timing, radar playback, lifestyle indices, premium/ad states, widgets, location prompts, push alerts, and regional forecast coverage.
...
- Phase 7: Conduct lawful hands-on verification for MinuteCast behavior, severe alert timing, radar playback, lifestyle indices, premium/ad states, widgets, location prompts, push alerts, and regional forecast coverage, then update this spec with evidence before any one-for-one parity claim.
