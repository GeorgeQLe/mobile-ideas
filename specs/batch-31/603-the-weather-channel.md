# The Weather Channel-Style Clone Spec

> Metadata
> - Inspiration app: The Weather Channel
...
> - Manual verification blockers: severe alert delivery, radar playback, video/news personalization, premium/ad removal, widgets, precise/background location prompts, push notification copy, and region-specific forecast coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by The Weather Channel's public user-facing workflow. The clone focus is local forecasts, severe weather alerts, radar maps, hourly and daily conditions, news/video surfaces, saved locations, widgets, premium/ad states, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.

## Goals

- Deliver a mobile-first maps weather outdoors experience centered on map, forecast, route, alert, sensor, and offline-planning workflow.
...
- Do not copy The Weather Channel branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/the-weather-channel-radar/id295646461 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.weather.Weather | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://weather.com/ | Public product orientation for local forecasts, severe weather alerts, radar maps, hourly and daily conditions, news/video surfaces, saved locations, widgets, premium/ad states, and location privacy controls | Public source verified |
| Help/support center | https://support.weather.com/ | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://weather.com/en-US/twc/privacy-policy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://weather.com/en-US/twc/terms-of-use | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |

## Detailed Design

- Source-backed requirement: the app must provide a mobile home surface that launches directly into map, forecast, route, alert, sensor, and offline-planning workflow.
...
- Manual verification required: severe alert delivery, radar playback, video/news personalization, premium/ad removal, widgets, precise/background location prompts, push notification copy, and region-specific forecast coverage.
...
- Safety requirement: precise location, saved places, route plans, GPS tracks, sensor readings, outdoor activity context, and alert history must be minimized, scoped, retained only as needed, and deleted/exported under user privacy requests unless retention is legally required.
...
- User starts map, forecast, route, alert, sensor, and offline-planning workflow, sees all important terms, and can cancel before irreversible commitment.
...
|---|---|---|---|---|
| Today Forecast | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Hourly | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Daily | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Radar Map | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Severe Alerts | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Saved Locations | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Video/News | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Health/Air Quality | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Widgets | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Premium | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ForecastLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `HourlyForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `DailyForecast`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `RadarFrame`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `SevereAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `SavedLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `WeatherStory`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `WidgetConfig`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for The Weather Channel-style workflows.
...
- Treat precise location, saved places, route plans, GPS tracks, sensor readings, outdoor activity context, and alert history as a launch-blocking privacy review area with owner, mitigation, retention policy, and acceptance tests.
- Treat subscriptions, premium map/weather layers, provider data access, purchases, refunds, alerts, saved places, and support cases as a launch-blocking financial/support review area with owner, mitigation, refund/dispute rules, and acceptance tests.
...
- Manual verification checklist tests for severe alert delivery, radar playback, video/news personalization, premium/ad removal, widgets, precise/background location prompts, push notification copy, and region-specific forecast coverage.
...
- A returning user can complete map, forecast, route, alert, sensor, and offline-planning workflow, recover from a network failure, and confirm canonical server state after reconnect.
...
- What retention periods apply to precise location, saved places, route plans, GPS tracks, sensor readings, outdoor activity context, and alert history, support cases, receipts, and audit records in each launch region?
...
- Phase 7: Conduct lawful hands-on verification for severe alert delivery, radar playback, video/news personalization, premium/ad removal, widgets, precise/background location prompts, push notification copy, and region-specific forecast coverage, then update this spec with evidence before any one-for-one parity claim.
