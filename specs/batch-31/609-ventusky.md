# Ventusky-Style Clone Spec

> Metadata
> - Inspiration app: Ventusky
...
> - Manual verification blockers: forecast model/layer behavior, timeline animation, premium layers, favorites sync, location prompts, push alerts where present, and regional forecast-source coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Ventusky's public user-facing workflow. The clone focus is animated global weather maps, model layers, wind and precipitation visualization, forecast timelines, favorites, premium/ad states, and location privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Ventusky branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/ventusky-weather-forecast/id1280984498 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=cz.ackee.ventusky | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.ventusky.com/ | Public product orientation for animated global weather maps, model layers, wind and precipitation visualization, forecast timelines, favorites, premium/ad states, and location privacy controls | Public source verified |
| Help/support center | https://www.ventusky.com/contact | Support taxonomy, account, subscriptions, alerts, saved items, provider handoff, contact, privacy, and recovery flows | Public source verified |
| Privacy policy | https://www.ventusky.com/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.ventusky.com/terms | Eligibility, acceptable use, subscription or lead terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: forecast model/layer behavior, timeline animation, premium layers, favorites sync, location prompts, push alerts where present, and regional forecast-source coverage.
...
| Timeline | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Wind Map | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Precipitation Map | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Premium | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Legal | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ForecastLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `WeatherLayer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `ForecastModel`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `TimelineFrame`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `MapViewport`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `FavoritePlace`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `ProviderAttribution`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
- `LocaleSetting`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Ventusky-style workflows.
...
- Manual verification checklist tests for forecast model/layer behavior, timeline animation, premium layers, favorites sync, location prompts, push alerts where present, and regional forecast-source coverage.
...
- Phase 7: Conduct lawful hands-on verification for forecast model/layer behavior, timeline animation, premium layers, favorites sync, location prompts, push alerts where present, and regional forecast-source coverage, then update this spec with evidence before any one-for-one parity claim.
