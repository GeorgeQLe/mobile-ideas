# Exodus-Style Clone Spec

> Metadata
> - Inspiration app: Exodus
...
Build an original mobile product inspired by Exodus's public product, support, policy, and marketplace materials. V1 focuses on self-custody portfolio, asset detail, exchange/swap blockers, staking blockers, wallet backup, support and security education. The clone must use original branding, original UI copy, synthetic sample data, licensed providers, and explicit disclosures whenever a requirement is inferred from public behavior rather than verified through lawful hands-on testing.
...
- Support self-custody portfolio, asset detail, exchange/swap blockers, staking blockers, wallet backup, support and security education with clear unavailable, pending, failed, blocked, held, canceled, expired, refunded, disputed, and recovered states.
...
- Do not imply affiliation with Exodus or its publisher.
...
|---|---|---|---|
| Official product | https://www.exodus.com/mobile/ | Public product positioning, core mobile workflow, availability framing, account/funding/payment surfaces, and support entry points | Verified 2026-05-04 |
| Support/help center | https://www.exodus.com/support/ | Public support taxonomy for onboarding, identity, funding, transfer/order/payment, dispute, refund, security, and account lifecycle flows | Verified 2026-05-04 |
| Privacy policy | https://www.exodus.com/legal/privacy-policy/ | Personal, financial, device, location, analytics, support, fraud-prevention, retention, and privacy-rights handling | Verified 2026-05-04 |
| Terms or user agreement | https://www.exodus.com/legal/exodus-terms-of-service/ | Eligibility, account obligations, prohibited use, fees, payment/transfer/credit/crypto risk disclosures, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/exodus-crypto-bitcoin-wallet/id1414384820 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=exodusmovement.exodus | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `IdentityCheck`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `RiskReview`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `PrivacyRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `Wallet`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `RecoveryPhraseCheckpoint`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `Network`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `TokenAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `WalletTransaction`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `DappConnection`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `SignatureRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `SwapQuote`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
- `NFTAsset`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Exodus-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, risk scores, or proprietary schemas from Exodus.
