# MetaMask-Style Clone Spec

> Metadata
> - Inspiration app: MetaMask
...
Build an original mobile product inspired by MetaMask's public product, support, policy, and marketplace materials. V1 focuses on wallet creation/import, seed phrase custody, network/token management, swaps, dapp browser/connect, signature preview, phishing warnings. The clone must use original branding, original UI copy, synthetic sample data, licensed providers, and explicit disclosures whenever a requirement is inferred from public behavior rather than verified through lawful hands-on testing.
...
- Support wallet creation/import, seed phrase custody, network/token management, swaps, dapp browser/connect, signature preview, phishing warnings with clear unavailable, pending, failed, blocked, held, canceled, expired, refunded, disputed, and recovered states.
...
- Do not imply affiliation with MetaMask or its publisher.
...
|---|---|---|---|
| Official product | https://metamask.io/download/ | Public product positioning, core mobile workflow, availability framing, account/funding/payment surfaces, and support entry points | Verified 2026-05-04 |
| Support/help center | https://support.metamask.io/ | Public support taxonomy for onboarding, identity, funding, transfer/order/payment, dispute, refund, security, and account lifecycle flows | Verified 2026-05-04 |
| Privacy policy | https://consensys.io/privacy-policy | Personal, financial, device, location, analytics, support, fraud-prevention, retention, and privacy-rights handling | Verified 2026-05-04 |
| Terms or user agreement | https://consensys.io/terms-of-use | Eligibility, account obligations, prohibited use, fees, payment/transfer/credit/crypto risk disclosures, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/metamask-blockchain-wallet/id1438144202 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=io.metamask | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `IdentityCheck`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `RiskReview`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `PrivacyRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `Wallet`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `RecoveryPhraseCheckpoint`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `Network`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `TokenAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `WalletTransaction`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `DappConnection`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `SignatureRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `SwapQuote`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
- `NFTAsset`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for MetaMask-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, risk scores, or proprietary schemas from MetaMask.
