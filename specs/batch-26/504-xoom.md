# Xoom-Style Clone Spec

> Metadata
> - Inspiration app: Xoom
...
Build an original mobile product inspired by Xoom's public product, support, policy, and marketplace materials. V1 focuses on PayPal-linked wallet funding, transfer quote, bank/cash/mobile-wallet payout, reload/top-up, delivery estimate, transaction protection. The clone must use original branding, original UI copy, synthetic sample data, licensed providers, and explicit disclosures whenever a requirement is inferred from public behavior rather than verified through lawful hands-on testing.
...
- Support PayPal-linked wallet funding, transfer quote, bank/cash/mobile-wallet payout, reload/top-up, delivery estimate, transaction protection with clear unavailable, pending, failed, blocked, held, canceled, expired, refunded, disputed, and recovered states.
...
- Do not imply affiliation with Xoom or its publisher.
...
|---|---|---|---|
| Official product | https://www.xoom.com/ | Public product positioning, core mobile workflow, availability framing, account/funding/payment surfaces, and support entry points | Verified 2026-05-04 |
| Support/help center | https://help.xoom.com/ | Public support taxonomy for onboarding, identity, funding, transfer/order/payment, dispute, refund, security, and account lifecycle flows | Verified 2026-05-04 |
| Privacy policy | https://www.paypal.com/us/legalhub/privacy-full | Personal, financial, device, location, analytics, support, fraud-prevention, retention, and privacy-rights handling | Verified 2026-05-04 |
| Terms or user agreement | https://www.xoom.com/legal/user-agreement | Eligibility, account obligations, prohibited use, fees, payment/transfer/credit/crypto risk disclosures, and legal boundaries | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/xoom-send-money-transfer/id529615515 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.xoom.android.app | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `IdentityCheck`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `RiskReview`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `PrivacyRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `Recipient`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `TransferQuote`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `FundingInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `PayoutMethod`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `Transfer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `FXRateSnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `DeliveryPartner`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `CancellationRequest`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `RefundCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
- `ComplianceHold`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, compliance flags, and audit metadata for Xoom-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, risk scores, or proprietary schemas from Xoom.
