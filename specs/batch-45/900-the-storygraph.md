# The StoryGraph-Style Clone Spec

> Metadata
> - Inspiration app: The StoryGraph
...
> - Manual verification blockers: StoryGraph account policy, book metadata source (Open Library, publisher data) and coverage, mood/pace/content-warning tagging methodology and crowdsourcing, recommendation algorithm (mood-based, content-based, collaborative filtering) and cold-start handling, reading progress tracking (page, percentage, time) and manual entry, TBR pile organization and prioritization, buddy read matching and scheduling, book club creation and discussion prompt generation, review system (star ratings, half-stars, text reviews) and spoiler handling, reading statistics calculation and visualization rendering, year-in-review generation and data completeness requirements, barcode scanning accuracy and ISBN lookup, Goodreads CSV import parsing and data mapping, StoryGraph Plus subscription entitlement (enhanced stats, content warnings, Plus-only features), social features (followers, activity feed) and privacy controls, dark mode and dynamic type support, accessibility features (VoiceOver, dynamic type), offline data caching, and API rate limits for book metadata lookup.
...
Build an original mobile product inspired by The StoryGraph's public books & reference workflow. The V1 clone focuses on book tracking and recommendation platform with reading log and progress tracking, mood-based and content-based book recommendations, personalized reading statistics and annual graphs, book search and detailed metadata (moods, pace, topics, content warnings), TBR (to-be-read) pile management, buddy reads and reading challenges, book clubs with discussion prompts, reviews and star/half-star ratings, reading journal and notes, year-in-review stats, barcode scanning for adding books, import from Goodreads, dark mode, and freemium model with StoryGraph Plus subscription.
...
- Preserve the functional job behind The StoryGraph while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy The StoryGraph branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/the-storygraph/id1570489264 | iOS listing, privacy labels, release notes, book tracking/recommendation claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.thestorygraph.thestorygraph | Android listing, data safety, book tracking/stats claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://www.thestorygraph.com/ | The StoryGraph book tracking, recommendations, stats, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://www.thestorygraph.com/faq | StoryGraph app help, features, import, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://www.thestorygraph.com/privacy | StoryGraph data collection, reading data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.thestorygraph.com/terms | StoryGraph services, content, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: StoryGraph account policy, book metadata source (Open Library, publisher data) and coverage, mood/pace/content-warning tagging methodology and crowdsourcing, recommendation algorithm (mood-based, content-based, collaborative filtering) and cold-start handling, reading progress tracking (page, percentage, time) and manual entry, TBR pile organization and prioritization, buddy read matching and scheduling, book club creation and discussion prompt generation, review system (star ratings, half-stars, text reviews) and spoiler handling, reading statistics calculation and visualization rendering, year-in-review generation and data completeness requirements, barcode scanning accuracy and ISBN lookup, Goodreads CSV import parsing and data mapping, StoryGraph Plus subscription entitlement (enhanced stats, content warnings, Plus-only features), social features (followers, activity feed) and privacy controls, dark mode and dynamic type support, accessibility features (VoiceOver, dynamic type), offline data caching, and API rate limits for book metadata lookup.

## Core User Journeys

- User creates or restores a The StoryGraph-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from The StoryGraph.
...
- Manual verification tests for StoryGraph account policy, book metadata source (Open Library, publisher data) and coverage, mood/pace/content-warning tagging methodology and crowdsourcing, recommendation algorithm (mood-based, content-based, collaborative filtering) and cold-start handling, reading progress tracking (page, percentage, time) and manual entry, TBR pile organization and prioritization, buddy read matching and scheduling, book club creation and discussion prompt generation, review system (star ratings, half-stars, text reviews) and spoiler handling, reading statistics calculation and visualization rendering, year-in-review generation and data completeness requirements, barcode scanning accuracy and ISBN lookup, Goodreads CSV import parsing and data mapping, StoryGraph Plus subscription entitlement (enhanced stats, content warnings, Plus-only features), social features (followers, activity feed) and privacy controls, dark mode and dynamic type support, accessibility features (VoiceOver, dynamic type), offline data caching, and API rate limits for book metadata lookup.
...
- Phase 4: Add app-specific safety controls for book tracking and recommendation platform with reading log and progress tracking, mood-based and content-based book recommendations, personalized reading statistics and annual graphs, book search and detailed metadata (moods, pace, topics, content warnings), TBR (to-be-read) pile management, buddy reads and reading challenges, book clubs with discussion prompts, reviews and star/half-star ratings, reading journal and notes, year-in-review stats, barcode scanning for adding books, import from Goodreads, dark mode, and freemium model with StoryGraph Plus subscription.
...
- Confirm legal/privacy retention behavior for The StoryGraph-style sensitive records and update API contracts before downstream implementation.
