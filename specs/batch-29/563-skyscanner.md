# Skyscanner-Style Clone Spec

> Metadata
> - Inspiration app: Skyscanner
...
> - Manual verification blockers: provider redirect accuracy, fare/fee freshness, price alert delivery, traveler profile state, booking partner support, payment handoff, and region/currency availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Skyscanner's public user-facing workflow. The clone focus is flight, hotel, and car search, price comparison, flexible-date exploration, alerts, booking-provider handoff, trip planning, and partner-price freshness. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Skyscanner branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/skyscanner-flights-travel/id415458524 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=net.skyscanner.android.main | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.skyscanner.com/ | Public product orientation for flight, hotel, and car search, price comparison, flexible-date exploration, alerts, booking-provider handoff, trip planning, and partner-price freshness | Public source verified |
| Help/support center | https://help.skyscanner.net/hc/en-us | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://www.skyscanner.net/privacy-policy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.skyscanner.net/terms-of-service | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: provider redirect accuracy, fare/fee freshness, price alert delivery, traveler profile state, booking partner support, payment handoff, and region/currency availability.
...
|---|---|---|---|---|
| Explore | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Flight Results | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Hotel Search | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Price Alerts | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Provider Handoff | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `SearchQuery`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `FlightOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `FareCalendar`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `HotelOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `CarOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `Provider`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `PriceAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `Trip`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `TravelerProfile`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
- `ReferralSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Skyscanner-style workflows.
...
- Manual verification checklist tests for provider redirect accuracy, fare/fee freshness, price alert delivery, traveler profile state, booking partner support, payment handoff, and region/currency availability.
...
- Phase 7: Conduct lawful hands-on verification for provider redirect accuracy, fare/fee freshness, price alert delivery, traveler profile state, booking partner support, payment handoff, and region/currency availability, then update this spec with evidence before any one-for-one parity claim.
