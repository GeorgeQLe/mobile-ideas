# momondo-Style Clone Spec

> Metadata
> - Inspiration app: momondo
...
> - Manual verification blockers: provider redirect accuracy, price/fee freshness, Trips sync, price-alert delivery, traveler profile, payment handoff, and regional/currency availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by momondo's public user-facing workflow. The clone focus is flight, hotel, and car metasearch, Explore budget discovery, price alerts, Trips itinerary organization, offline trip access, and provider handoff. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy momondo branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/momondo-flights-hotels-cars/id436736538 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.momondo.flightsearch | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.momondo.com/ | Public product orientation for flight, hotel, and car metasearch, Explore budget discovery, price alerts, Trips itinerary organization, offline trip access, and provider handoff | Public source verified |
| Help/support center | https://www.momondo.com/help | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://www.momondo.com/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.momondo.com/terms-of-use | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: provider redirect accuracy, price/fee freshness, Trips sync, price-alert delivery, traveler profile, payment handoff, and regional/currency availability.
...
| Saved Search | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `SearchQuery`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `FlightOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `StayOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `CarOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `Trip`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `ItineraryItem`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `BudgetExploreOption`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `PriceAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `Provider`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
- `ReferralSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for momondo-style workflows.
...
- Manual verification checklist tests for provider redirect accuracy, price/fee freshness, Trips sync, price-alert delivery, traveler profile, payment handoff, and regional/currency availability.
...
- Phase 7: Conduct lawful hands-on verification for provider redirect accuracy, price/fee freshness, Trips sync, price-alert delivery, traveler profile, payment handoff, and regional/currency availability, then update this spec with evidence before any one-for-one parity claim.
