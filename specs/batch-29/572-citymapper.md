# Citymapper-Style Clone Spec

> Metadata
> - Inspiration app: Citymapper
...
> - Manual verification blockers: GO navigation, transit/ride-share/scooter availability, fare estimates, live disruption freshness, location prompts, push alerts, subscription/ad behavior, and city-specific coverage require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Citymapper's public user-facing workflow. The clone focus is multimodal city route comparison, live transit navigation, walking/cycling/scooter routing, disruption data, saved places, fare-aware options, and city coverage. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Citymapper branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/citymapper/id469463298 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.citymapper.app.release | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://citymapper.com/ | Public product orientation for multimodal city route comparison, live transit navigation, walking/cycling/scooter routing, disruption data, saved places, fare-aware options, and city coverage | Public source verified |
| Help/support center | https://intercom.help/citymapper/en/ | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://citymapper.com/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://citymapper.com/terms | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: GO navigation, transit/ride-share/scooter availability, fare estimates, live disruption freshness, location prompts, push alerts, subscription/ad behavior, and city-specific coverage.
...
|---|---|---|---|---|
| City Home | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| GO Navigation | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Line Status | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Nearby Stops | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Bike/Scooter Map | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `City`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `RouteQuery`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `RouteOption`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `TransportLeg`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `Stop`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `LineStatus`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `SharedMobilityVehicle`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `FareEstimate`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `SavedPlace`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
- `ServiceAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Citymapper-style workflows.
...
- Manual verification checklist tests for GO navigation, transit/ride-share/scooter availability, fare estimates, live disruption freshness, location prompts, push alerts, subscription/ad behavior, and city-specific coverage.
...
- Phase 7: Conduct lawful hands-on verification for GO navigation, transit/ride-share/scooter availability, fare estimates, live disruption freshness, location prompts, push alerts, subscription/ad behavior, and city-specific coverage, then update this spec with evidence before any one-for-one parity claim.
