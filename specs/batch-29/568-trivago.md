# trivago-Style Clone Spec

> Metadata
> - Inspiration app: trivago
...
> - Manual verification blockers: provider price freshness, mobile-rate eligibility, booking-site redirect, price-drop alert delivery, aggregated review provenance, region/currency coverage, and support routing require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by trivago's public user-facing workflow. The clone focus is hotel price comparison, accommodation filters, mobile-rate discovery, price-drop alerts, favorite comparison, aggregated reviews, and booking-site handoff. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy trivago branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/trivago-compare-hotel-prices/id376888389 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.trivago | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.trivago.com/ | Public product orientation for hotel price comparison, accommodation filters, mobile-rate discovery, price-drop alerts, favorite comparison, aggregated reviews, and booking-site handoff | Public source verified |
| Help/support center | https://support.trivago.com/hc/en-us | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://company.trivago.com/privacy-policy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://company.trivago.com/terms-and-conditions/ | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: provider price freshness, mobile-rate eligibility, booking-site redirect, price-drop alert delivery, aggregated review provenance, region/currency coverage, and support routing.
...
| Offer Comparison | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Provider Handoff | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Price Drop Alert | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Review Summary | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `SearchQuery`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `Accommodation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `HotelOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `Provider`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `Favorite`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `PriceAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `ReviewAggregate`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `FilterSet`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `ReferralSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for trivago-style workflows.
...
- Manual verification checklist tests for provider price freshness, mobile-rate eligibility, booking-site redirect, price-drop alert delivery, aggregated review provenance, region/currency coverage, and support routing.
...
- Phase 7: Conduct lawful hands-on verification for provider price freshness, mobile-rate eligibility, booking-site redirect, price-drop alert delivery, aggregated review provenance, region/currency coverage, and support routing, then update this spec with evidence before any one-for-one parity claim.
