...
> - Manual verification blockers: service-zone eligibility, shared-route assignment, live vehicle tracking, fare/pass purchase, accessibility accommodations, driver/support handoff, payment/refund states, and local program rules require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Via's public user-facing workflow. The clone focus is on-demand shared ride booking, corner pickup/dropoff, service-zone eligibility, live vehicle tracking, fares, passes, accessibility support, receipts, and local program rules. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Via branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/via-smarter-mobility/id657777015 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=via.rider | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://ridewithvia.com/ | Public product orientation for on-demand shared ride booking, corner pickup/dropoff, service-zone eligibility, live vehicle tracking, fares, passes, accessibility support, receipts, and local program rules | Public source verified |
| Help/support center | https://support.ridewithvia.com/hc/en-us | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://ridewithvia.com/privacy-policy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://ridewithvia.com/terms-of-use/ | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: service-zone eligibility, shared-route assignment, live vehicle tracking, fare/pass purchase, accessibility accommodations, driver/support handoff, payment/refund states, and local program rules.
...
|---|---|---|---|---|
| Service Zone | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Pickup/Dropoff | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Ride Options | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Booking Confirm | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Walk To Pickup | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Fare/Passes | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `ServiceZone`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `RideRequest`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `SharedRide`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `PickupPoint`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `Vehicle`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `FareQuote`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `Pass`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `PaymentIntent`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `Receipt`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Via-style workflows.
...
- Manual verification checklist tests for service-zone eligibility, shared-route assignment, live vehicle tracking, fare/pass purchase, accessibility accommodations, driver/support handoff, payment/refund states, and local program rules.
...
- Phase 7: Conduct lawful hands-on verification for service-zone eligibility, shared-route assignment, live vehicle tracking, fare/pass purchase, accessibility accommodations, driver/support handoff, payment/refund states, and local program rules, then update this spec with evidence before any one-for-one parity claim.
