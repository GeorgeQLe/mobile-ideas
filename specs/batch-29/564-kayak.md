# KAYAK-Style Clone Spec

> Metadata
> - Inspiration app: KAYAK
...
> - Manual verification blockers: Trips inbox sync, flight tracker alerts, bag-measure camera flow, provider redirect accuracy, price-change behavior, payment handoff, and regional partner inventory require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by KAYAK's public user-facing workflow. The clone focus is travel metasearch across flights, hotels, cars, vacation rentals, trips, price alerts, flight tracking, offline itinerary access, and provider handoff. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy KAYAK branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/kayak-flights-hotels-cars/id305204535 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.kayak.android | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.kayak.com/ | Public product orientation for travel metasearch across flights, hotels, cars, vacation rentals, trips, price alerts, flight tracking, offline itinerary access, and provider handoff | Public source verified |
| Help/support center | https://www.kayak.com/help | Support taxonomy, account, booking, cancellation, refund, ticketing, trip, and recovery flows | Public source verified |
| Privacy policy | https://www.kayak.com/privacy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.kayak.com/terms-of-use | Eligibility, acceptable use, booking/payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: Trips inbox sync, flight tracker alerts, bag-measure camera flow, provider redirect accuracy, price-change behavior, payment handoff, and regional partner inventory.
...
| Hotel Results | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Car Results | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Explore | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Flight Tracker | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Price Alert | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Bag Measure | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `SearchQuery`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `FlightOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `StayOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `CarOffer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `Trip`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `ItineraryItem`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `PriceAlert`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `TrackedFlight`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `Provider`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
- `ReferralSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for KAYAK-style workflows.
...
- Manual verification checklist tests for Trips inbox sync, flight tracker alerts, bag-measure camera flow, provider redirect accuracy, price-change behavior, payment handoff, and regional partner inventory.
...
- Phase 7: Conduct lawful hands-on verification for Trips inbox sync, flight tracker alerts, bag-measure camera flow, provider redirect accuracy, price-change behavior, payment handoff, and regional partner inventory, then update this spec with evidence before any one-for-one parity claim.
