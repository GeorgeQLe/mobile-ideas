# AliExpress-Style Clone Spec

> Metadata
> - Inspiration app: AliExpress
...
Build an original mobile product inspired by AliExpress's public product and policy materials. V1 focuses on global marketplace search, seller disclosure, product variants, coupons, cart/checkout, cross-border shipping estimates, order tracking, buyer protection, returns/refunds, taxes/duties blockers, and support. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support global marketplace search, seller disclosure, product variants, coupons, cart/checkout, cross-border shipping estimates, order tracking, buyer protection, returns/refunds, taxes/duties blockers, and support with clear retry, recovery, unavailable, region, entitlement, inventory, seller, and provider-aware states.
...
- Do not imply affiliation with AliExpress or its publisher.
...
|---|---|---|---|
| Official product | https://www.aliexpress.us/ | Public product positioning, shopping, delivery, catalog, marketplace, membership, account, and app behavior | Verified 2026-05-04 |
| Support/help center | https://helpcenter.aliexpress.com/ | Public support taxonomy for account, ordering, payment, pickup, shipping, returns, seller, loyalty, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://rulechannel.alibaba.com/icbu?type=detail&ruleId=2034 | Account, profile, order, listing, location, device, advertising, analytics, support, seller, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://rulechannel.alibaba.com/icbu?type=detail&ruleId=2042 | Service usage, licensed content, payments, returns, marketplace conduct, seller/buyer limits, prohibited goods, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or seller program | https://sale.aliexpress.com/ | Reward, membership, loyalty, seller, service, authenticity, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/aliexpress/id436672029 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.alibaba.aliexpresshd | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- global marketplace search, seller disclosure, product variants, coupons, cart/checkout, cross-border shipping estimates, order tracking, buyer protection, returns/refunds, taxes/duties blockers, and support must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Store`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Address`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Product`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `ProductVariant`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `InventorySnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `PriceOffer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `LoyaltyAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Wishlist`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Cart`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `PaymentInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Order`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Fulfillment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `ReturnCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `Review`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `ServiceAppointment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for AliExpress-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from AliExpress.
