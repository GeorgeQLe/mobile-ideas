# Craigslist-Style Clone Spec

> Metadata
> - Inspiration app: Craigslist
...
Build an original mobile product inspired by Craigslist's public product and policy materials. V1 focuses on local classifieds city selection, category browsing, posting/reply flows, saved searches, map/list views, alerts, housing/jobs/service category constraints, prohibited-content moderation, and scam controls. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support local classifieds city selection, category browsing, posting/reply flows, saved searches, map/list views, alerts, housing/jobs/service category constraints, prohibited-content moderation, and scam controls with clear retry, recovery, unavailable, region, entitlement, inventory, seller, and provider-aware states.
- Preserve boundaries between buyer, seller, poster, listing, city, category, reply, saved search, alert, map, moderation, safety, fee, housing, jobs, services, and classifieds data.
...
- Do not imply affiliation with Craigslist or its publisher.
...
|---|---|---|---|
| Official product | https://www.craigslist.org/ | Public product positioning, shopping, delivery, catalog, marketplace, membership, account, and app behavior | Verified 2026-05-04 |
| Support/help center | https://www.craigslist.org/about/help/ | Public support taxonomy for account, ordering, payment, pickup, shipping, returns, seller, loyalty, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.craigslist.org/about/privacy.policy | Account, profile, order, listing, location, device, advertising, analytics, support, seller, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.craigslist.org/about/terms.of.use | Service usage, licensed content, payments, returns, marketplace conduct, seller/buyer limits, prohibited goods, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or seller program | https://www.craigslist.org/about/help/posting_fees | Reward, membership, loyalty, seller, service, authenticity, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/craigslist/id1336642410 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=org.craigslist.CraigslistMobile | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- local classifieds city selection, category browsing, posting/reply flows, saved searches, map/list views, alerts, housing/jobs/service category constraints, prohibited-content moderation, and scam controls must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
- scams, housing/job discrimination, prohibited listings, adult or unsafe content, user harassment, location privacy, reply anonymization, posting fees, moderation accuracy, and law-enforcement or legal request handling must be treated as a launch-blocking risk area with owner, mitigation, and acceptance tests.
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `City`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `Category`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `Listing`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `ListingMedia`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `ReplyAlias`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `SavedSearch`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `SearchAlert`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `MapLocation`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `PostingFee`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `ModerationCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `SafetyReport`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Craigslist-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Craigslist.
