...
> - Verification basis: exact public first-party product, help/support, privacy, terms, and marketplace pages listed below.
> - Manual verification blockers: native iOS/Android screen capture, app-store privacy labels and release notes, account lifecycle walkthrough, paid subscription or quota state, push notification prompts, media/file permission prompts, provider integrations, and region-specific behavior still require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, screenshots, proprietary filters/presets/effects, model weights, private APIs, customer media, marketplace assets, licensed stock, and unlicensed training or evaluation datasets.

## Overview

Build an original mobile ai assistant product inspired by Consensus's public product and policy materials. V1 focuses on research-question intake, paper search, source-backed AI summaries, citations, evidence ratings, saved research, history, and subscription/API gates. The clone must use original branding, original UI copy, original sample content, licensed providers/assets, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support prompt, attachment, citation, answer, saved thread, and provider-routing workflows with offline-safe drafts and clear retry/recovery behavior.
...
- Implement free, trial, paid, expired, restored, refunded, quota-exhausted, and unavailable entitlement states without copying exact pricing, plan names, or promotional copy.
...
- Document provider, asset, AI, media, subscription, and marketplace risks before any implementation claim.

## Non-Goals

- Do not imply affiliation with Consensus or its publisher.
- Do not copy proprietary presets, filters, brush engines, templates, stickers, model weights, marketplace assets, screenshots, icons, brand names, rankings, or private API shapes.
...
- Do not present generated, edited, or source-backed output as professional, medical, legal, financial, scientific, therapeutic, educational, or safety-critical advice.
- Do not enable autonomous external actions, purchases, publication, account changes, or regulated workflows without a separate confirmation and risk review.
...
|---|---|---|---|
| Official product | https://consensus.app/ | AI research search product surface, source-backed answers, papers, and account entry | Verified 2026-05-03 |
| Help center | https://help.consensus.app/ | Public support taxonomy for search, accounts, subscriptions, API, and security | Verified 2026-05-03 |
| Privacy policy | https://www.consensus.com/privacy-policy/ | Account, query, analytics, security, vendor, and retention data handling | Verified 2026-05-03 |
| Terms of service | https://www.consensus.com/terms-of-service/ | Acceptable use, subscriptions, API use, citations, and legal boundaries | Verified 2026-05-03 |
| Security help | https://help.consensus.app/en/articles/10093291-security | Public statement on responsible AI, privacy, and anonymized usage handling | Verified 2026-05-03 |
...
- The home surface must expose recent conversations/searches, templates or starters, a primary create action, and degraded offline/account states.
- The create flow must support prompt entry, model/source selection, attachment upload, citation inspection, regenerate, copy, share, delete, and report controls.
- Answers must show source/citation or unsupported-claim indicators where applicable, stream or complete with partial-failure states, and preserve user review before consequential use.
...
- Account settings must include privacy policy, terms, notification preferences, subscription restore/manage, support, data export, delete account, and AI/media data-use controls where supported.
...
- Analytics must avoid raw prompts, generated answers, citations, uploaded media, face geometry, photos/videos, drawings, transcripts, payment credentials, exact contacts, and child data.
...
- Source citation reliability, paper metadata licensing, medical/scientific claim disclaimers, query retention, API key handling, and academic-integrity boundaries require explicit controls.
...
- Returning user resumes a recent conversation or research query, completes the primary workflow, and exports or saves the result.
...
- User applies a premium model, tool, preset, template, brush, effect, asset, or quota-limited feature and sees a clear entitlement state.
- User loses connectivity during generation/search and can preserve the draft, retry safely, or reconcile duplicated work after reconnect.
...
- Manual verification required: native purchase/restore, app-store privacy labels, permission prompts, push notifications, background processing, and exact mobile UI states.
...
| Create / Import | Start primary workflow | prompt, camera, photo, file | idle, importing, ready | permission denied, unsupported type, oversized file |
| Editor / Composer | Main creation workspace | tools, text, gestures, model | drafting, processing, saved | provider timeout, tool unavailable, corrupt draft |
| Tool / Model Picker | Capability routing | mode, effect, model, preset | available, selected | plan locked, provider down, region locked |
| Asset Library | Templates, presets, stickers, papers | browse, search, select | loaded, filtered | license unavailable, removed asset |
| Preview / Result | Inspect output before save/share | compare, copy, export | complete, partial, watermarked | unsafe output, low quality, missing source |
...
- `Conversation`: owner, title, source type, draft state, retention policy, pinned/archive/deleted state, and export eligibility.
- `Message`: content reference, edit/generation status, tool/model metadata, safety labels, asset refs, and deletion state.
...
- `AssetLicense`: template/preset/sticker/brush/source owner, usage scope, commercial-use flag, attribution, expiry, and takedown state.
...
- Drafts, selected tools, local edit stacks, and export settings cache locally with explicit size, retention, and purge rules.
- Long-running AI/edit/export jobs use polling, SSE, or websocket fallback and must support cancel, timeout, retry, and canonical refetch after reconnect.
...
- Background export, upload, camera capture, and cloud sync behavior is `Manual verification required` and must stay disabled until platform-specific evidence exists.
- Cached prompts, uploads, originals, previews, generated outputs, edited exports, and asset manifests purge on logout, account delete, retention expiry, workspace policy change, or legal hold.

## Permissions, Privacy, And Safety

- Request camera, microphone, speech recognition, photo library, files, clipboard, contacts/share sheet, notifications, or workspace OAuth only at feature use.
...
- Do not collect raw prompts, answers, citations, photos, videos, drawings, face geometry, EXIF, transcripts, files, or exports in analytics or crash logs.
...
- Media imports must strip or let users control EXIF, GPS, face tags, camera serials, and embedded private metadata before export/share.
- AI, beauty, face, body, object, background, and identity edits must block non-consensual sexualization, child imagery, deepfake impersonation, doxxing, harassment, and illegal content.
...
- Analytics events: onboarding started/completed, import started/completed, tool selected, edit applied, generation completed/failed, export completed, report submitted, export/delete requested, and paywall viewed.
- Event properties must use coarse file type classes, tool categories, model capability class, latency buckets, error codes, output dimensions buckets, and entitlement state only.
- Monetization may gate premium tools, higher quotas, batch exports, cloud sync, stock/templates, commercial-use assets, AI generation, high-resolution output, watermark removal, team features, or priority processing.
...
- User imports a huge file, unsupported format, corrupt image, low-storage video, or media with sensitive EXIF/location.
- Provider outage after the user submits an edit/generation job but before output is persisted.
- User cancels processing after credits or quota are consumed.
- Tool, model, preset, brush, template, or asset becomes unavailable, renamed, region-locked, or takedown-blocked between draft and export.
- Generated or edited output includes unsafe, infringing, non-consensual, misleading, or missing-attribution content.
- Draft is deleted on one device while another device is exporting or processing it.
- Subscription renews, refunds, or expires while a premium export/generation is in progress.
- Account/team policy disables AI, cloud sync, commercial assets, or public sharing after content has been cached locally.
...
- User attempts to use generated/edited output for identity deception, financial/scientific claims, regulated advertising, or unauthorized commercial use.
...
- Unit tests for import validation, MIME/size limits, EXIF policy, scan states, retention flags, and deletion/export eligibility.
- Unit tests for edit stack undo/redo, destructive-confirmation gates, asset license propagation, watermark state, and export metadata policy.
...
- Integration tests for onboarding -> import/create -> edit/generate -> preview -> export/share -> delete.
- Integration tests for permission denied, granted, revoked, limited photo access, oversized files, unsupported inputs, and provider timeouts.
...
- Safety tests for hallucination/source disclaimers where applicable, face/body/beauty edits, sexual/deepfake abuse, child-safety prompts, copyright reports, and moderation states.
...
- All source-discovery links are replaced with exact first-party product/help/privacy/terms or marketplace URLs, or an explicit manual blocker.
- A lawful V1 can be built with original branding, UI copy, sample content, providers, and licensed assets.
- Onboarding, home/studio, import/create, editor/composer, tool/model picker, asset library, preview/result, export/share, settings, support, safety report, export/delete, and entitlement screens are specified.
...
- Safety tests cover hallucinations or source claims where relevant, face/body/media abuse, non-consensual edits, copyright reports, generated-content attribution, and commercial-use warnings.
...
- Which provider(s) will power AI, image processing, media storage, moderation, stock/template libraries, billing, analytics, and notifications in the original implementation?
...
- Phase 2: Implement auth, home/studio, import/create, editor/composer shell, project history, settings, and support routes.
- Phase 3: Add tool/model routing, asset library, edit/generation jobs, preview/result states, quota states, and provider error handling.
...
- Phase 5: Add billing/restore, privacy controls, analytics redaction, provider retention controls, accessibility, media safety, and policy tests.
...
- Select original providers for media processing, AI generation, moderation, storage, billing, analytics, and notifications.
