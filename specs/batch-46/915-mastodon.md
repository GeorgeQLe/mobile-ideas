# Mastodon-Style Clone Spec

> Metadata
> - Inspiration app: Mastodon
> - Category: Social
...
> - Manual verification blockers: Mastodon account and instance selection (server choice affects community, rules, moderation), ActivityPub federation protocol implementation and cross-instance compatibility, home/local/federated timeline construction and caching, post composition with media attachments (image, video, audio, polls) and content warnings, boost and favorite federation across instances, hashtag indexing and trending algorithm (instance-local vs federated), profile verification (rel=me link verification) and migration between instances, notification delivery across federated network, direct message privacy (not end-to-end encrypted, visible to instance admins), list management and timeline filtering, bookmark sync (instance-local, not federated), instance selection UX and instance discovery/recommendation, moderation tools (mute, block, report, domain block) and instance-level moderation, content filter (keyword/phrase filtering by context), media description enforcement and accessibility, multi-account/multi-instance management, push notification delivery (Web Push protocol per instance), instance ToS and community rules variation, API compatibility (Mastodon API vs other ActivityPub implementations), and open-source self-hosting considerations.
...
Build an original mobile product inspired by Mastodon's public social workflow. The V1 clone focuses on decentralized social networking platform with federated timeline (ActivityPub protocol), home timeline with posts from followed accounts, local and federated timelines, posting (toots) with text, images, video, audio, polls, and content warnings, boost (reblog) and favorite interactions, hashtag discovery and trending topics, profile customization with bio, avatar, header, and verification links, notifications (mentions, boosts, favorites, follows, polls), direct messages, lists for organizing followed accounts, bookmarks and saved posts, server/instance selection and migration, moderation tools (mute, block, report, filter), accessibility features (image descriptions, reduced motion), multi-account support, push notifications, and free open-source model (instance-hosted).
...
- Preserve the functional job behind Mastodon while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Mastodon branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/mastodon-for-iphone-and-ipad/id1571998974 | iOS listing, privacy labels, release notes, decentralized social claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=org.joinmastodon.android | Android listing, data safety, decentralized social/federation claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://joinmastodon.org/ | Mastodon decentralized social, server selection, features, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://docs.joinmastodon.org/ | Mastodon documentation, user guide, admin guide, and API reference | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://joinmastodon.org/privacy-policy | Mastodon project privacy, instance-level data handling, and disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://joinmastodon.org/covenant | Mastodon Server Covenant, moderation standards, and community expectations | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Mastodon account and instance selection (server choice affects community, rules, moderation), ActivityPub federation protocol implementation and cross-instance compatibility, home/local/federated timeline construction and caching, post composition with media attachments (image, video, audio, polls) and content warnings, boost and favorite federation across instances, hashtag indexing and trending algorithm (instance-local vs federated), profile verification (rel=me link verification) and migration between instances, notification delivery across federated network, direct message privacy (not end-to-end encrypted, visible to instance admins), list management and timeline filtering, bookmark sync (instance-local, not federated), instance selection UX and instance discovery/recommendation, moderation tools (mute, block, report, domain block) and instance-level moderation, content filter (keyword/phrase filtering by context), media description enforcement and accessibility, multi-account/multi-instance management, push notification delivery (Web Push protocol per instance), instance ToS and community rules variation, API compatibility (Mastodon API vs other ActivityPub implementations), and open-source self-hosting considerations.

## Core User Journeys

- User creates or restores a Mastodon-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Mastodon.
...
- Manual verification tests for Mastodon account and instance selection (server choice affects community, rules, moderation), ActivityPub federation protocol implementation and cross-instance compatibility, home/local/federated timeline construction and caching, post composition with media attachments (image, video, audio, polls) and content warnings, boost and favorite federation across instances, hashtag indexing and trending algorithm (instance-local vs federated), profile verification (rel=me link verification) and migration between instances, notification delivery across federated network, direct message privacy (not end-to-end encrypted, visible to instance admins), list management and timeline filtering, bookmark sync (instance-local, not federated), instance selection UX and instance discovery/recommendation, moderation tools (mute, block, report, domain block) and instance-level moderation, content filter (keyword/phrase filtering by context), media description enforcement and accessibility, multi-account/multi-instance management, push notification delivery (Web Push protocol per instance), instance ToS and community rules variation, API compatibility (Mastodon API vs other ActivityPub implementations), and open-source self-hosting considerations.
...
- Phase 4: Add app-specific safety controls for decentralized social networking platform with federated timeline (ActivityPub protocol), home timeline with posts from followed accounts, local and federated timelines, posting (toots) with text, images, video, audio, polls, and content warnings, boost (reblog) and favorite interactions, hashtag discovery and trending topics, profile customization with bio, avatar, header, and verification links, notifications (mentions, boosts, favorites, follows, polls), direct messages, lists for organizing followed accounts, bookmarks and saved posts, server/instance selection and migration, moderation tools (mute, block, report, filter), accessibility features (image descriptions, reduced motion), multi-account support, push notifications, and free open-source model (instance-hosted).
...
- Confirm legal/privacy retention behavior for Mastodon-style sensitive records and update API contracts before downstream implementation.
