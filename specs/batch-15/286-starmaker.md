# StarMaker-Style Clone Spec

> Metadata
> - Inspiration app: StarMaker
...
Build an original mobile karaoke and social audio product inspired by StarMaker's public product and policy materials. V1 focuses on karaoke singing, live/social rooms, duet recordings, song discovery, creator profiles, virtual items, and moderation controls. The clone must use original branding, original UI copy, original sample media, licensed providers/assets, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support karaoke singing, live/social rooms, duet recordings, song discovery, creator profiles, virtual items, and moderation controls with clear retry, recovery, unavailable, and rights-aware states.
...
- Do not imply affiliation with StarMaker or its publisher.
...
|---|---|---|---|
| Official product | https://www.starmakerstudios.com/ | Public StarMaker product positioning, karaoke, creator, and social surfaces | Verified 2026-05-04 |
| Help/support | https://support.starmakerstudios.com/ | Public support taxonomy for accounts, subscriptions, safety, reporting, and troubleshooting | Verified 2026-05-04 |
| Privacy policy | https://www.starmakerstudios.com/privacy | Account, recordings, social graph, device, analytics, retention, and child-safety handling | Verified 2026-05-04 |
| Terms of service | https://www.starmakerstudios.com/terms | User content, subscriptions, virtual items, acceptable use, and legal boundaries | Verified 2026-05-04 |
...
- Onboarding must support signed-out preview, account creation or restore, blocked-account state, unavailable-region state, and entitlement-unavailable state for karaoke and social audio workflows.
- Home must expose the latest meaningful karaoke and social audio state, search/discovery, library or project access, settings, and degraded offline/account variants.
...
|---|---|---|---|---|
| Welcome / Auth | Defines the welcome / auth workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Home / Feed | Defines the home / feed workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Songbook Search | Defines the songbook search workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Song Detail | Defines the song detail workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Record / Perform | Defines the record / perform workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Duet / Group Invite | Defines the duet / group invite workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Effects / Mix | Defines the effects / mix workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Preview / Publish | Defines the preview / publish workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Profile | Defines the profile workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Comments / Messages | Defines the comments / messages workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| VIP Subscription | Defines the vip subscription workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Notifications | Defines the notifications workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Settings / Privacy | Defines the settings / privacy workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Safety / Reports | Defines the safety / reports workflow for StarMaker-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |

## Data Model

- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `LibraryItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `CatalogOrFeedItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `CreatorOrSource`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `PlaybackSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `QueueOrProject`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `MediaAsset`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `ImportOrUpload`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `ProviderJob`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `RightsOrSafetyReport`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for StarMaker-inspired karaoke and social audio workflows.
