# Smule-Style Clone Spec

> Metadata
> - Inspiration app: Smule
...
Build an original mobile karaoke and social audio product inspired by Smule's public product and policy materials. V1 focuses on karaoke singing, duet/group recordings, songbook discovery, performance publishing, comments, creator profiles, and VIP subscription gates. The clone must use original branding, original UI copy, original sample media, licensed providers/assets, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support karaoke singing, duet/group recordings, songbook discovery, performance publishing, comments, creator profiles, and VIP subscription gates with clear retry, recovery, unavailable, and rights-aware states.
...
- Do not imply affiliation with Smule or its publisher.
...
|---|---|---|---|
| Official product | https://www.smule.com/ | Public karaoke, songbook, profile, creator, and social performance surfaces | Verified 2026-05-04 |
| Help center | https://smule.zendesk.com/hc/en-us | Public support taxonomy for accounts, songs, subscriptions, safety, and troubleshooting | Verified 2026-05-04 |
| Privacy policy | https://www.smule.com/privacy | Account, recordings, voice/video, device, usage, social, retention, and child-safety handling | Verified 2026-05-04 |
| Terms of service | https://www.smule.com/termsofservice | User content, music rights, subscriptions, community behavior, and legal boundaries | Verified 2026-05-04 |
...
|---|---|---|---|---|
| Welcome / Auth | Defines the welcome / auth workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Home / Feed | Defines the home / feed workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Songbook Search | Defines the songbook search workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Song Detail | Defines the song detail workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Record / Perform | Defines the record / perform workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Duet / Group Invite | Defines the duet / group invite workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Effects / Mix | Defines the effects / mix workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Preview / Publish | Defines the preview / publish workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Profile | Defines the profile workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Comments / Messages | Defines the comments / messages workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| VIP Subscription | Defines the vip subscription workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Notifications | Defines the notifications workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Settings / Privacy | Defines the settings / privacy workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |
| Safety / Reports | Defines the safety / reports workflow for Smule-inspired karaoke and social audio behavior | taps, forms, search, deep links, permissions, media controls | empty, loading, loaded, signed-out, offline |

## Data Model

- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `LibraryItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `CatalogOrFeedItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `CreatorOrSource`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `PlaybackSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `QueueOrProject`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `MediaAsset`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `ImportOrUpload`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `ProviderJob`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `Entitlement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `NotificationPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `RightsOrSafetyReport`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Smule-inspired karaoke and social audio workflows.
