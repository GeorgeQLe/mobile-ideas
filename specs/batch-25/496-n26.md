...
Build an original mobile product inspired by N26's public product and policy materials. V1 focuses on secure digital banking onboarding, account dashboard, card controls, transfers, spaces/budgeting, insights, subscription-tier blockers, statements, support, fraud controls, and regional compliance blockers. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support secure digital banking onboarding, account dashboard, card controls, transfers, spaces/budgeting, insights, subscription-tier blockers, statements, support, fraud controls, and regional compliance blockers with clear retry, recovery, unavailable, region, entitlement, inventory, seller, and provider-aware states.
...
- Do not imply affiliation with N26 or its publisher.
...
|---|---|---|---|
| Official product | https://n26.com/en-eu/app | Public product positioning, shopping, delivery, catalog, marketplace, membership, account, and app behavior | Verified 2026-05-04 |
| Support/help center | https://support.n26.com/ | Public support taxonomy for account, ordering, payment, pickup, shipping, returns, seller, loyalty, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://n26.com/en-eu/privacy-policy | Account, profile, order, listing, location, device, advertising, analytics, support, seller, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://n26.com/en-eu/legal-documents | Service usage, licensed content, payments, returns, marketplace conduct, seller/buyer limits, prohibited goods, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or seller program | https://n26.com/en-eu/plans | Reward, membership, loyalty, seller, service, authenticity, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/n26-mobile-banking/id956857223 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=de.number26.android | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- secure digital banking onboarding, account dashboard, card controls, transfers, spaces/budgeting, insights, subscription-tier blockers, statements, support, fraud controls, and regional compliance blockers must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `IdentityCheck`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `BankAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `CardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `BalanceSnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `Transaction`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `Transfer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `BillPayment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `MobileDeposit`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `CardControl`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `AlertPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `Statement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `RewardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `DisputeCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `FraudCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for N26-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from N26.
