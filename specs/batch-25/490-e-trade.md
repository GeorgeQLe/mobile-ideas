# E*TRADE-Style Clone Spec

> Metadata
> - Inspiration app: E*TRADE
...
Build an original mobile product inspired by E*TRADE's public product and policy materials. V1 focuses on secure brokerage sign-in, portfolio dashboard, watchlists, trading-ticket simulation, options and margin blockers, market-data licensing, alerts, banking handoffs, support, and compliance controls. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support secure brokerage sign-in, portfolio dashboard, watchlists, trading-ticket simulation, options and margin blockers, market-data licensing, alerts, banking handoffs, support, and compliance controls with clear retry, recovery, unavailable, region, entitlement, inventory, seller, and provider-aware states.
...
- Do not imply affiliation with E*TRADE or its publisher.
...
|---|---|---|---|
| Official product | https://us.etrade.com/platforms/mobile | Public product positioning, shopping, delivery, catalog, marketplace, membership, account, and app behavior | Verified 2026-05-04 |
| Support/help center | https://us.etrade.com/contact-us | Public support taxonomy for account, ordering, payment, pickup, shipping, returns, seller, loyalty, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://us.etrade.com/l/privacy-statement | Account, profile, order, listing, location, device, advertising, analytics, support, seller, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://us.etrade.com/l/terms-and-conditions | Service usage, licensed content, payments, returns, marketplace conduct, seller/buyer limits, prohibited goods, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or seller program | https://us.etrade.com/pricing | Reward, membership, loyalty, seller, service, authenticity, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/e-trade-invest-trade-save/id313259740 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.etrade.mobilepro.activity | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- secure brokerage sign-in, portfolio dashboard, watchlists, trading-ticket simulation, options and margin blockers, market-data licensing, alerts, banking handoffs, support, and compliance controls must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `IdentityCheck`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `BankAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `CardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `BalanceSnapshot`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `Transaction`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `Transfer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `BillPayment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `MobileDeposit`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `CardControl`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `AlertPreference`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `Statement`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `RewardAccount`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `DisputeCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `FraudCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for E*TRADE-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from E*TRADE.
