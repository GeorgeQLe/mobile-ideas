...
Build an original mobile product inspired by Goodpods's public user-facing workflow. The clone focus is: Catalog discovery, playback queue, library saves, downloads/offline behavior, creator/channel surfaces, and subscription or rights gates

This Draft 1 spec reserves ID 312 in the 1000-app backlog. It provides enough structure for downstream research, estimation, and lawful implementation planning, but it is not implementation-ready until exact first-party URLs replace source-discovery links and app-specific public evidence is added.

## Goals

- Deliver a mobile-first podcast and audio experience with onboarding, primary workflow, settings, support, and recovery flows.
- Reproduce the functional job behind Goodpods using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Goodpods branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/search?term=Goodpods | iOS listing, category, age rating, privacy labels, release notes, support links | Source discovery -- pending exact URL verification |
| Google Play | https://play.google.com/store/search?q=Goodpods&c=apps | Android listing, content rating, data safety, feature blurbs | Source discovery -- pending exact URL verification |
| Official website/help search | https://www.google.com/search?q=Goodpods%20official%20app%20help%20privacy%20terms | Help center, privacy, terms, support, subscription, safety, and product docs to replace with first-party URLs | Source discovery -- pending exact URL verification |
...
- `User`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `CatalogItem`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `CreatorOrSource`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `Collection`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `PlaybackOrReadSession`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `QueueItem`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `Download`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `Entitlement`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `Comment`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
- `RecommendationSlot`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Goodpods-style workflows.
