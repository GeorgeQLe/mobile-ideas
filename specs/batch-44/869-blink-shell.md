# Blink Shell-Style Clone Spec

> Metadata
> - Inspiration app: Blink Shell
...
> - Manual verification blockers: Blink Shell purchase/subscription and Blink+ feature gating, SSH connection authentication (password, key, certificate, FIDO2/security key), Mosh UDP session resumption and state synchronization, local shell command availability and sandbox limitations (iOS), VS Code Server/code-server tunnel establishment and WebSocket reliability, tmux integration and session restore after app termination, SSH key storage in Secure Enclave and iCloud Keychain sync, iCloud Drive and Files app provider integration for file access, custom font installation and theme CSS configuration, hardware keyboard shortcut mapping and modifier key customization, Split View and Stage Manager multi-window session management, Blink Build cloud VM provisioning and SSH tunnel, snippet variable interpolation and execution, geo-location connection profile switching, iPad trackpad/mouse support and cursor behavior, push notification delivery for build/connection events, offline configuration access, and Apple ecosystem integration (iCloud sync, Shortcuts, Handoff).
...
Build an original mobile product inspired by Blink Shell's public developer tools workflow. The V1 clone focuses on professional iOS terminal and shell with SSH and Mosh connections, local shell access, code editor integration (VS Code Server, code-server), tmux-style session persistence, SSH key management and agent forwarding, iCloud and Files app integration, custom fonts and themes (configurable appearance), hardware keyboard optimization with shortcuts, split view and multiple windows (Stage Manager, iPad), Blink Build (cloud development environments), snippets and command shortcuts, geo-location-aware connections, push notifications for build/connection events, and one-time purchase with optional subscription (Blink+).
...
- Preserve the functional job behind Blink Shell while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Blink Shell branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/blink-shell-build-code/id1594898306 | iOS listing, privacy labels, release notes, terminal/SSH/code claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://blink.sh/ | Blink Shell terminal, SSH, Mosh, code editor, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://docs.blink.sh/ | Blink Shell documentation, SSH setup, keys, themes, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://blink.sh/privacy-policy | Blink Shell account, connection data, iCloud sync, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://blink.sh/terms-of-use | Blink Shell services, purchases, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Blink Shell purchase/subscription and Blink+ feature gating, SSH connection authentication (password, key, certificate, FIDO2/security key), Mosh UDP session resumption and state synchronization, local shell command availability and sandbox limitations (iOS), VS Code Server/code-server tunnel establishment and WebSocket reliability, tmux integration and session restore after app termination, SSH key storage in Secure Enclave and iCloud Keychain sync, iCloud Drive and Files app provider integration for file access, custom font installation and theme CSS configuration, hardware keyboard shortcut mapping and modifier key customization, Split View and Stage Manager multi-window session management, Blink Build cloud VM provisioning and SSH tunnel, snippet variable interpolation and execution, geo-location connection profile switching, iPad trackpad/mouse support and cursor behavior, push notification delivery for build/connection events, offline configuration access, and Apple ecosystem integration (iCloud sync, Shortcuts, Handoff).

## Core User Journeys

- User creates or restores a Blink Shell-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Blink Shell.
...
- Manual verification tests for Blink Shell purchase/subscription and Blink+ feature gating, SSH connection authentication (password, key, certificate, FIDO2/security key), Mosh UDP session resumption and state synchronization, local shell command availability and sandbox limitations (iOS), VS Code Server/code-server tunnel establishment and WebSocket reliability, tmux integration and session restore after app termination, SSH key storage in Secure Enclave and iCloud Keychain sync, iCloud Drive and Files app provider integration for file access, custom font installation and theme CSS configuration, hardware keyboard shortcut mapping and modifier key customization, Split View and Stage Manager multi-window session management, Blink Build cloud VM provisioning and SSH tunnel, snippet variable interpolation and execution, geo-location connection profile switching, iPad trackpad/mouse support and cursor behavior, push notification delivery for build/connection events, offline configuration access, and Apple ecosystem integration (iCloud sync, Shortcuts, Handoff).
...
- Phase 4: Add app-specific safety controls for professional iOS terminal and shell with SSH and Mosh connections, local shell access, code editor integration (VS Code Server, code-server), tmux-style session persistence, SSH key management and agent forwarding, iCloud and Files app integration, custom fonts and themes (configurable appearance), hardware keyboard optimization with shortcuts, split view and multiple windows (Stage Manager, iPad), Blink Build (cloud development environments), snippets and command shortcuts, geo-location-aware connections, push notifications for build/connection events, and one-time purchase with optional subscription (Blink+).
...
- Confirm legal/privacy retention behavior for Blink Shell-style sensitive records and update API contracts before downstream implementation.
