# Working Copy-Style Clone Spec

> Metadata
> - Inspiration app: Working Copy
...
> - Manual verification blockers: Working Copy purchase and Pro feature gating, Git repository clone performance and large-repo handling, commit creation with author/committer identity and GPG signing, diff rendering for large files and binary content, branch operations (create, checkout, merge, rebase, delete) and conflict resolution UI, staging area granularity (file, hunk, line) and interactive rebase, remote authentication (SSH key, HTTPS credential, GitHub/GitLab/Bitbucket token), push/pull/fetch error handling and force-push confirmation, syntax highlighting language detection and editor performance, image diff (side-by-side, overlay) and binary file handling, Files app provider registration and document access, Shortcuts actions (clone, pull, commit, push, status) and Automation triggers, share extension payload handling and quick-commit workflow, iPad Split View and drag-and-drop between apps, external editor round-trip via URL scheme and file coordination, LFS support and partial clone, submodule initialization and update, push notification delivery for webhook-triggered events, offline repository browsing, and iCloud/local storage management.
...
Build an original mobile product inspired by Working Copy's public developer tools workflow. The V1 clone focuses on Git client for iOS/iPad with full repository cloning and management, commit history and diff viewer, branch and tag management, merge and rebase operations, staging area with per-hunk staging, remote management (push, pull, fetch), SSH and HTTPS authentication, syntax-highlighted file editor, image and binary file preview, Files app integration (repository as file provider), Shortcuts and Automation support, share extension for quick commits, split view for iPad multitasking, external editor integration (launch in other apps), push notifications for repository events, and one-time purchase with pro unlock.
...
- Preserve the functional job behind Working Copy while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Working Copy branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/working-copy-git-client/id896694807 | iOS listing, privacy labels, release notes, Git client claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://workingcopy.app/ | Working Copy Git client, features, integrations, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://workingcopy.app/manual/ | Working Copy manual, repository setup, remotes, editor, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://workingcopy.app/privacy/ | Working Copy account, repository data, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://workingcopy.app/terms/ | Working Copy services, purchases, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Working Copy purchase and Pro feature gating, Git repository clone performance and large-repo handling, commit creation with author/committer identity and GPG signing, diff rendering for large files and binary content, branch operations (create, checkout, merge, rebase, delete) and conflict resolution UI, staging area granularity (file, hunk, line) and interactive rebase, remote authentication (SSH key, HTTPS credential, GitHub/GitLab/Bitbucket token), push/pull/fetch error handling and force-push confirmation, syntax highlighting language detection and editor performance, image diff (side-by-side, overlay) and binary file handling, Files app provider registration and document access, Shortcuts actions (clone, pull, commit, push, status) and Automation triggers, share extension payload handling and quick-commit workflow, iPad Split View and drag-and-drop between apps, external editor round-trip via URL scheme and file coordination, LFS support and partial clone, submodule initialization and update, push notification delivery for webhook-triggered events, offline repository browsing, and iCloud/local storage management.

## Core User Journeys

- User creates or restores a Working Copy-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Working Copy.
...
- Manual verification tests for Working Copy purchase and Pro feature gating, Git repository clone performance and large-repo handling, commit creation with author/committer identity and GPG signing, diff rendering for large files and binary content, branch operations (create, checkout, merge, rebase, delete) and conflict resolution UI, staging area granularity (file, hunk, line) and interactive rebase, remote authentication (SSH key, HTTPS credential, GitHub/GitLab/Bitbucket token), push/pull/fetch error handling and force-push confirmation, syntax highlighting language detection and editor performance, image diff (side-by-side, overlay) and binary file handling, Files app provider registration and document access, Shortcuts actions (clone, pull, commit, push, status) and Automation triggers, share extension payload handling and quick-commit workflow, iPad Split View and drag-and-drop between apps, external editor round-trip via URL scheme and file coordination, LFS support and partial clone, submodule initialization and update, push notification delivery for webhook-triggered events, offline repository browsing, and iCloud/local storage management.
...
- Phase 4: Add app-specific safety controls for Git client for iOS/iPad with full repository cloning and management, commit history and diff viewer, branch and tag management, merge and rebase operations, staging area with per-hunk staging, remote management (push, pull, fetch), SSH and HTTPS authentication, syntax-highlighted file editor, image and binary file preview, Files app integration (repository as file provider), Shortcuts and Automation support, share extension for quick commits, split view for iPad multitasking, external editor integration (launch in other apps), push notifications for repository events, and one-time purchase with pro unlock.
...
- Confirm legal/privacy retention behavior for Working Copy-style sensitive records and update API contracts before downstream implementation.
