# Termius-Style Clone Spec

> Metadata
> - Inspiration app: Termius
...
> - Manual verification blockers: Termius account and team policy, personal/team/business plan feature gating, SSH connection authentication methods (password, key, certificate, agent forwarding), host discovery and manual/imported configuration, SSH key generation (RSA, Ed25519, ECDSA) and passphrase handling, Mosh UDP connection and roaming support, port forwarding tunnel lifecycle and binding rules, SFTP directory listing and file transfer progress/resume, snippet variable interpolation and execution context, terminal emulation compatibility (xterm-256color, VT100), multi-session tab management and split-view layout, cloud sync encryption and conflict resolution, team vault credential sharing and access revocation, autocomplete suggestion source and command history, connection keep-alive and reconnect behavior, proxy/jump-host chaining and bastion configuration, push notification delivery for connection/sync events, offline host list access, API rate limits, and SSO/SAML provisioning.
...
Build an original mobile product inspired by Termius's public developer tools workflow. The V1 clone focuses on SSH/SFTP client with multi-protocol terminal access (SSH, Mosh, Telnet, serial), host and group organization with labels and tags, identity and key management (SSH keys, certificates), port forwarding (local, remote, dynamic SOCKS), SFTP file browser and transfer, snippet library for command shortcuts, terminal appearance customization (themes, fonts, cursor), tabbed sessions and split view, cloud sync of hosts/keys/snippets across devices, team sharing of hosts and credentials, autocomplete and command suggestions, push notifications for connection status, and plan-gated feature enforcement (Free, Pro, Teams, Business).
...
- Preserve the functional job behind Termius while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Termius branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/termius-terminal-ssh-client/id549039908 | iOS listing, privacy labels, release notes, SSH/terminal claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.server.auditor.ssh.client | Android listing, data safety, SSH/SFTP/terminal claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://termius.com/ | Termius SSH client, terminal, SFTP, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://support.termius.com/ | Termius documentation, SSH setup, sync, snippets, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://termius.com/privacy-policy | Termius account, connection data, sync data, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://termius.com/terms-of-service | Termius services, subscriptions, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Termius account and team policy, personal/team/business plan feature gating, SSH connection authentication methods (password, key, certificate, agent forwarding), host discovery and manual/imported configuration, SSH key generation (RSA, Ed25519, ECDSA) and passphrase handling, Mosh UDP connection and roaming support, port forwarding tunnel lifecycle and binding rules, SFTP directory listing and file transfer progress/resume, snippet variable interpolation and execution context, terminal emulation compatibility (xterm-256color, VT100), multi-session tab management and split-view layout, cloud sync encryption and conflict resolution, team vault credential sharing and access revocation, autocomplete suggestion source and command history, connection keep-alive and reconnect behavior, proxy/jump-host chaining and bastion configuration, push notification delivery for connection/sync events, offline host list access, API rate limits, and SSO/SAML provisioning.

## Core User Journeys

- User creates or restores a Termius-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
|---|---|---|---|---|
| Welcome/Auth | Account login or purchase verification, device pairing, server connection, and permission primers | email, OAuth, purchase-restore, QR-scan, key-import | new, returning, paired, unlicensed | auth failed, purchase not found, connection refused, timeout |
| Sessions/Connections | Active terminal sessions, SSH/Mosh connections, project list, server host management, and connection status | connect, disconnect, new-session, switch, configure | connected, disconnected, connecting, error, idle | connection dropped, auth rejected, host unreachable, timeout |
| Editor/Terminal | Code editor or terminal emulator with syntax highlighting, command execution, split view, and file navigation | type, execute, navigate, split, resize, paste | active, idle, running-command, error, read-only | process killed, buffer overflow, encoding error, permission denied |
| Files/Repository | File browser, Git repository status, staging area, diff viewer, commit history, and branch management | browse, stage, commit, push, pull, diff, checkout | clean, modified, staged, conflict, detached | merge conflict, push rejected, auth expired, large-file warning |
| Build/Preview | Build output, app preview, development server status, logs, and deployment/update management | build, run, preview, refresh, stop, deploy | building, running, stopped, failed, previewing | build failed, preview crash, server unreachable, bundle too large |
| Settings/Privacy | Account, connections/hosts, keys, appearance (themes/fonts), sync, notifications, privacy, and support | forms, toggle, import, export, configure | loaded, editing, syncing | sync conflict, key invalid, purchase expired, storage full |
...
- Treat source code and repository contents, SSH private keys and authentication credentials, terminal session history and command output, server host addresses and connection configurations, SFTP file transfers and remote file contents, Git commit metadata and author identities, development project files and build artifacts, cloud sync encryption keys, team-shared credentials and vault data, device-local storage and cached code, iCloud/cloud backup contents, and purchase/subscription verification tokens as launch-blocking review areas with owners, mitigations, and acceptance tests before implementation.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Termius.
...
- Manual verification tests for Termius account and team policy, personal/team/business plan feature gating, SSH connection authentication methods (password, key, certificate, agent forwarding), host discovery and manual/imported configuration, SSH key generation (RSA, Ed25519, ECDSA) and passphrase handling, Mosh UDP connection and roaming support, port forwarding tunnel lifecycle and binding rules, SFTP directory listing and file transfer progress/resume, snippet variable interpolation and execution context, terminal emulation compatibility (xterm-256color, VT100), multi-session tab management and split-view layout, cloud sync encryption and conflict resolution, team vault credential sharing and access revocation, autocomplete suggestion source and command history, connection keep-alive and reconnect behavior, proxy/jump-host chaining and bastion configuration, push notification delivery for connection/sync events, offline host list access, API rate limits, and SSO/SAML provisioning.
...
- Phase 4: Add app-specific safety controls for SSH/SFTP client with multi-protocol terminal access (SSH, Mosh, Telnet, serial), host and group organization with labels and tags, identity and key management (SSH keys, certificates), port forwarding (local, remote, dynamic SOCKS), SFTP file browser and transfer, snippet library for command shortcuts, terminal appearance customization (themes, fonts, cursor), tabbed sessions and split view, cloud sync of hosts/keys/snippets across devices, team sharing of hosts and credentials, autocomplete and command suggestions, push notifications for connection status, and plan-gated feature enforcement (Free, Pro, Teams, Business).
...
- Confirm legal/privacy retention behavior for Termius-style sensitive records and update API contracts before downstream implementation.
