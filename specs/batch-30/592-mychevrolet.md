# myChevrolet-Style Clone Spec

> Metadata
> - Inspiration app: myChevrolet
...
> - Manual verification blockers: vehicle pairing, OnStar/connected service eligibility, remote command reliability, subscription/trial services, EV charging data, roadside support, safety-critical controls, payment state, and country availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by myChevrolet's public user-facing workflow. The clone focus is GM/Chevrolet account onboarding, remote commands, vehicle status, EV charging, service scheduling, roadside support, owner resources, notifications, and privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy myChevrolet branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/mychevrolet/id398596699 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.gm.chevrolet.nomad.ownership | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.chevrolet.com/support/vehicle/mobile-apps/mychevrolet-app | Public product orientation for GM/Chevrolet account onboarding, remote commands, vehicle status, EV charging, service scheduling, roadside support, owner resources, notifications, and privacy controls | Public source verified |
| Help/support center | https://www.chevrolet.com/support | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.gm.com/privacy-statement | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.gm.com/terms-of-use | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: vehicle pairing, OnStar/connected service eligibility, remote command reliability, subscription/trial services, EV charging data, roadside support, safety-critical controls, payment state, and country availability.
...
- `OwnerAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `Vehicle`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `VehiclePairing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `RemoteCommand`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `VehicleStatus`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `LocationPing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `ServiceAppointment`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `ConnectedServiceEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for myChevrolet-style workflows.
...
- Manual verification checklist tests for vehicle pairing, OnStar/connected service eligibility, remote command reliability, subscription/trial services, EV charging data, roadside support, safety-critical controls, payment state, and country availability.
...
- Phase 7: Conduct lawful hands-on verification for vehicle pairing, OnStar/connected service eligibility, remote command reliability, subscription/trial services, EV charging data, roadside support, safety-critical controls, payment state, and country availability, then update this spec with evidence before any one-for-one parity claim.
