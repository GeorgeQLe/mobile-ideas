...
Build an original mobile product inspired by onX Hunt's public user-facing workflow. The clone focus is: Map or forecast discovery, saved places, route/activity planning, offline mode, alerts, sensors, and location privacy gates

This Draft 1 spec reserves ID 598 in the 1000-app backlog. It provides enough structure for downstream research, estimation, and lawful implementation planning, but it is not implementation-ready until exact first-party URLs replace source-discovery links and app-specific public evidence is added.
...
- Reproduce the functional job behind onX Hunt using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy onX Hunt branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/search?term=onX%20Hunt | iOS listing, category, age rating, privacy labels, release notes, support links | Source discovery -- pending exact URL verification |
| Google Play | https://play.google.com/store/search?q=onX%20Hunt&c=apps | Android listing, content rating, data safety, feature blurbs | Source discovery -- pending exact URL verification |
| Official website/help search | https://www.google.com/search?q=onX%20Hunt%20official%20app%20help%20privacy%20terms | Help center, privacy, terms, support, subscription, safety, and product docs to replace with first-party URLs | Source discovery -- pending exact URL verification |
...
- `User`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Location`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Listing`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Availability`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Booking`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `PaymentIntent`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `MessageThread`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Route`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `Review`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
- `SupportCase`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for onX Hunt-style workflows.
