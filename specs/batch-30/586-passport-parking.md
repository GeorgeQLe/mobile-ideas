# Passport Parking-Style Clone Spec

> Metadata
> - Inspiration app: Passport Parking
...
> - Manual verification blockers: municipal zone rules, meter/session enforcement, extension availability, convenience fees, citation/refund handling, payment recovery, and city availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Passport Parking's public user-facing workflow. The clone focus is zone lookup, paid parking sessions, session extension, reminders, payment methods, history, email receipts, support, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Passport Parking branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/passport-parking/id501324867 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.passportparking.mobile | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.passportparking.com/ | Public product orientation for zone lookup, paid parking sessions, session extension, reminders, payment methods, history, email receipts, support, and location privacy | Public source verified |
| Help/support center | https://passportinc.com/support/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://passportinc.com/privacy-policy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://passportinc.com/terms-and-conditions/ | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: municipal zone rules, meter/session enforcement, extension availability, convenience fees, citation/refund handling, payment recovery, and city availability.
...
- `DriverAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `ParkingZone`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `ParkingFacility`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `ParkingSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `Reservation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `PaymentMethod`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `Receipt`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `Reminder`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Passport Parking-style workflows.
...
- Manual verification checklist tests for municipal zone rules, meter/session enforcement, extension availability, convenience fees, citation/refund handling, payment recovery, and city availability.
...
- Phase 7: Conduct lawful hands-on verification for municipal zone rules, meter/session enforcement, extension availability, convenience fees, citation/refund handling, payment recovery, and city availability, then update this spec with evidence before any one-for-one parity claim.
