# FordPass-Style Clone Spec

> Metadata
> - Inspiration app: FordPass
...
> - Manual verification blockers: vehicle pairing, FordPass Connect hardware, phone-as-a-key, remote command reliability, subscription/trial services, charging network data, roadside support, safety-critical controls, and country availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by FordPass's public user-facing workflow. The clone focus is Ford account onboarding, remote start/lock/unlock, vehicle status, GPS location, EV charging, service scheduling, connected services, roadside support, notifications, and privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy FordPass branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/fordpass/id1095418609 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.ford.fordpass | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.ford.com/support/category/fordpass/ | Public product orientation for Ford account onboarding, remote start/lock/unlock, vehicle status, GPS location, EV charging, service scheduling, connected services, roadside support, notifications, and privacy controls | Public source verified |
| Help/support center | https://www.ford.com/support/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.ford.com/help/privacy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.ford.com/help/terms/ | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: vehicle pairing, FordPass Connect hardware, phone-as-a-key, remote command reliability, subscription/trial services, charging network data, roadside support, safety-critical controls, and country availability.
...
- `OwnerAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `Vehicle`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `VehiclePairing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `RemoteCommand`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `VehicleStatus`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `LocationPing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `ServiceAppointment`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `ConnectedServiceEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for FordPass-style workflows.
...
- Manual verification checklist tests for vehicle pairing, FordPass Connect hardware, phone-as-a-key, remote command reliability, subscription/trial services, charging network data, roadside support, safety-critical controls, and country availability.
...
- Phase 7: Conduct lawful hands-on verification for vehicle pairing, FordPass Connect hardware, phone-as-a-key, remote command reliability, subscription/trial services, charging network data, roadside support, safety-critical controls, and country availability, then update this spec with evidence before any one-for-one parity claim.
