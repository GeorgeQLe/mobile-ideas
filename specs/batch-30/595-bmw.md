...
> - Manual verification blockers: vehicle pairing, BMW ConnectedDrive eligibility, digital key, remote command reliability, charging session data, subscription services, service booking, safety-critical controls, and regional availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by BMW's public user-facing workflow. The clone focus is My BMW account onboarding, vehicle pairing, remote lock/climate, vehicle status, map/send destination, charging, service scheduling, digital key, notifications, and privacy controls. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy BMW branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/my-bmw/id1087277146 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=de.bmw.connected.mobile20.na | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.bmwusa.com/explore/connecteddrive.html | Public product orientation for My BMW account onboarding, vehicle pairing, remote lock/climate, vehicle status, map/send destination, charging, service scheduling, digital key, notifications, and privacy controls | Public source verified |
| Help/support center | https://faq.bmwusa.com/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.bmwusa.com/privacy-policy.html | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.bmwusa.com/terms-and-conditions.html | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: vehicle pairing, BMW ConnectedDrive eligibility, digital key, remote command reliability, charging session data, subscription services, service booking, safety-critical controls, and regional availability.
...
- `OwnerAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `Vehicle`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `VehiclePairing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `RemoteCommand`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `VehicleStatus`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `LocationPing`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `ServiceAppointment`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `ConnectedServiceEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for BMW-style workflows.
...
- Manual verification checklist tests for vehicle pairing, BMW ConnectedDrive eligibility, digital key, remote command reliability, charging session data, subscription services, service booking, safety-critical controls, and regional availability.
...
- Phase 7: Conduct lawful hands-on verification for vehicle pairing, BMW ConnectedDrive eligibility, digital key, remote command reliability, charging session data, subscription services, service booking, safety-critical controls, and regional availability, then update this spec with evidence before any one-for-one parity claim.
