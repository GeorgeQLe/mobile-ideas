# Electrify America-Style Clone Spec

> Metadata
> - Inspiration app: Electrify America
...
> - Manual verification blockers: live charger availability, membership enrollment, NFC/app session start, payment authorization, charger faults, support unlocks, refunds, vehicle compatibility, and US network availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Electrify America's public user-facing workflow. The clone focus is DC fast charger discovery, live charger availability, Pass/Pass+ membership, station details, favorite stations, charging session start/stop, status tracking, notifications, payments, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.

## Goals

- Deliver a mobile-first experience centered on charging discovery and charging-session flow.
...
- Do not copy Electrify America branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/electrify-america/id1458030456 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.ea.evowner | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.electrifyamerica.com/mobile-app/ | Public product orientation for DC fast charger discovery, live charger availability, Pass/Pass+ membership, station details, favorite stations, charging session start/stop, status tracking, notifications, payments, and location privacy | Public source verified |
| Help/support center | https://www.electrifyamerica.com/contact-us/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.electrifyamerica.com/privacy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.electrifyamerica.com/terms/ | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |

## Detailed Design

- Source-backed requirement: the app must provide a mobile home surface that launches directly into charging discovery and charging-session flow.
...
- Manual verification required: live charger availability, membership enrollment, NFC/app session start, payment authorization, charger faults, support unlocks, refunds, vehicle compatibility, and US network availability.
...
- Safety requirement: station location, charging activity, payment methods, vehicle compatibility, and session history data must be minimized, scoped, retained only as needed, and deleted/exported under user privacy requests unless retention is legally required.
...
- User starts charging discovery and charging-session flow, sees all important terms or warnings, and can cancel before irreversible commitment.
...
| Charging Map | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Station Results | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Session Start | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Active Charging | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Payment/Wallet | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Charging History | Trust, account, recovery, or support flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `DriverAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `ChargingStation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `Connector`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `StationAvailability`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `ChargingSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `PaymentMethod`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `Membership`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `Receipt`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Electrify America-style workflows.
...
- Treat station location, charging activity, payment methods, vehicle compatibility, and session history as a launch-blocking privacy review area with owner, mitigation, retention policy, and acceptance tests.
...
- Manual verification checklist tests for live charger availability, membership enrollment, NFC/app session start, payment authorization, charger faults, support unlocks, refunds, vehicle compatibility, and US network availability.
...
- A returning user can complete charging discovery and charging-session flow, recover from a network failure, and confirm canonical server state after reconnect.
...
- What retention periods apply to station location, charging activity, payment methods, vehicle compatibility, and session history, support cases, receipts, and audit records in each launch region?
...
- Phase 7: Conduct lawful hands-on verification for live charger availability, membership enrollment, NFC/app session start, payment authorization, charger faults, support unlocks, refunds, vehicle compatibility, and US network availability, then update this spec with evidence before any one-for-one parity claim.
