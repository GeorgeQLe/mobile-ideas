# SpotHero-Style Clone Spec

> Metadata
> - Inspiration app: SpotHero
...
> - Manual verification blockers: live garage availability, gate scanner acceptance, overstay handling, business/expense integrations, refunds, parking operator disputes, payment states, and city/event availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by SpotHero's public user-facing workflow. The clone focus is parking search, garage/lot comparison, prepaid reservation passes, QR/barcode entry instructions, business profiles, expense receipts, support, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy SpotHero branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/spothero-find-parking/id499097243 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.spothero.spothero | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://spothero.com/ | Public product orientation for parking search, garage/lot comparison, prepaid reservation passes, QR/barcode entry instructions, business profiles, expense receipts, support, and location privacy | Public source verified |
| Help/support center | https://support.spothero.com/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://spothero.com/privacy-policy | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://spothero.com/terms-of-use | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: live garage availability, gate scanner acceptance, overstay handling, business/expense integrations, refunds, parking operator disputes, payment states, and city/event availability.
...
- `DriverAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `ParkingZone`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `ParkingFacility`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `ParkingSession`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `Reservation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `PaymentMethod`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `Receipt`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `Reminder`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `NotificationPreference`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for SpotHero-style workflows.
...
- Manual verification checklist tests for live garage availability, gate scanner acceptance, overstay handling, business/expense integrations, refunds, parking operator disputes, payment states, and city/event availability.
...
- Phase 7: Conduct lawful hands-on verification for live garage availability, gate scanner acceptance, overstay handling, business/expense integrations, refunds, parking operator disputes, payment states, and city/event availability, then update this spec with evidence before any one-for-one parity claim.
