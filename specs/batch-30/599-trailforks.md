# Trailforks-Style Clone Spec

> Metadata
> - Inspiration app: Trailforks
...
> - Manual verification blockers: trail database licensing, offline region limits, trail condition moderation, subscription entitlements, Garmin/Strava sync, emergency sharing behavior, background location, and regional map availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Trailforks's public user-facing workflow. The clone focus is trail map discovery, route planning, trail condition reports, offline map downloads, GPS/activity tracking, events/weather alerts, Pro subscriptions, emergency sharing, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Trailforks branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/trailforks/id987986743 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.pinkbike.trailforks | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.trailforks.com/apps/map/ | Public product orientation for trail map discovery, route planning, trail condition reports, offline map downloads, GPS/activity tracking, events/weather alerts, Pro subscriptions, emergency sharing, and location privacy | Public source verified |
| Help/support center | https://www.trailforks.com/help/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.trailforks.com/about/privacy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.trailforks.com/about/terms/ | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: trail database licensing, offline region limits, trail condition moderation, subscription entitlements, Garmin/Strava sync, emergency sharing behavior, background location, and regional map availability.
...
- `ExplorerAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `MapLayer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `TrailOrRoute`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `Waypoint`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `OfflineRegion`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `TrackRecording`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `SharedRoute`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `SafetyNotice`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Trailforks-style workflows.
...
- Manual verification checklist tests for trail database licensing, offline region limits, trail condition moderation, subscription entitlements, Garmin/Strava sync, emergency sharing behavior, background location, and regional map availability.
...
- Phase 7: Conduct lawful hands-on verification for trail database licensing, offline region limits, trail condition moderation, subscription entitlements, Garmin/Strava sync, emergency sharing behavior, background location, and regional map availability, then update this spec with evidence before any one-for-one parity claim.
