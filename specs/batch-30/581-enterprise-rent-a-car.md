# Enterprise Rent-A-Car-Style Clone Spec

> Metadata
> - Inspiration app: Enterprise Rent-A-Car
...
> - Manual verification blockers: driver eligibility, rental counter pickup, check-in identity details, deposits, damage/incident handling, rewards redemption, payment/refund states, roadside assistance, and country availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Enterprise Rent-A-Car's public user-facing workflow. The clone focus is rental location search, vehicle class comparison, reservation management, check-in, return changes, roadside assistance, rewards, receipts, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.

## Goals

- Deliver a mobile-first experience centered on rental booking and trip-management flow.
...
- Do not copy Enterprise Rent-A-Car branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/enterprise-rent-a-car/id1020641417 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.ehi.enterprise.android | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.enterprise.com/ | Public product orientation for rental location search, vehicle class comparison, reservation management, check-in, return changes, roadside assistance, rewards, receipts, and location privacy | Public source verified |
| Help/support center | https://www.enterprise.com/en/help.html | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://privacy.ehi.com/en-us/home.html | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.enterprise.com/en/car-rental/terms-and-conditions.html | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |

## Detailed Design

- Source-backed requirement: the app must provide a mobile home surface that launches directly into rental booking and trip-management flow.
...
- Manual verification required: driver eligibility, rental counter pickup, check-in identity details, deposits, damage/incident handling, rewards redemption, payment/refund states, roadside assistance, and country availability.
...
- Safety requirement: rental pickup/dropoff, driver identity, payment methods, deposits, roadside incidents, and trip history data must be minimized, scoped, retained only as needed, and deleted/exported under user privacy requests unless retention is legally required.
...
- User starts rental booking and trip-management flow, sees all important terms or warnings, and can cancel before irreversible commitment.
...
| Location Search | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Vehicle Results | Core discovery and decision flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
| Reservation | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Check-In | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
| Pickup/Return | Transaction, live state, or management flow | taps, forms, filters, map gestures, deep links, permissions | empty, loading, loaded, signed-out, saved, unavailable | denied permission, stale inventory, offline, region-locked, provider error, blocked entitlement |
...
- `RenterAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `RentalLocation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `VehicleClass`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `Reservation`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `DriverEligibility`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `PickupReturnState`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `PaymentIntent`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `Receipt`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `RewardAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Enterprise Rent-A-Car-style workflows.
...
- Treat rental pickup/dropoff, driver identity, payment methods, deposits, roadside incidents, and trip history as a launch-blocking privacy review area with owner, mitigation, retention policy, and acceptance tests.
...
- Manual verification checklist tests for driver eligibility, rental counter pickup, check-in identity details, deposits, damage/incident handling, rewards redemption, payment/refund states, roadside assistance, and country availability.
...
- A returning user can complete rental booking and trip-management flow, recover from a network failure, and confirm canonical server state after reconnect.
...
- What retention periods apply to rental pickup/dropoff, driver identity, payment methods, deposits, roadside incidents, and trip history, support cases, receipts, and audit records in each launch region?
...
- Phase 7: Conduct lawful hands-on verification for driver eligibility, rental counter pickup, check-in identity details, deposits, damage/incident handling, rewards redemption, payment/refund states, roadside assistance, and country availability, then update this spec with evidence before any one-for-one parity claim.
