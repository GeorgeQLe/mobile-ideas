# Gaia GPS-Style Clone Spec

> Metadata
> - Inspiration app: Gaia GPS
...
> - Manual verification blockers: GPS accuracy, offline map download limits, subscription entitlements, third-party map layer licenses, emergency/safety disclaimers, route import/export, background location, and regional map availability require lawful test evidence before one-for-one native parity claims.
> - Legal scope: functional parity only; use original code, brand, copy, iconography, sample data, media, policies, and integrations.

## Overview

Build an original mobile product inspired by Gaia GPS's public user-facing workflow. The clone focus is topographic map discovery, route planning, saved waypoints, offline maps, GPS recording, weather/layers, subscription access, sharing, and location privacy. This public-source V1 spec is implementation-ready for an original build, but every behavior marked `Manual verification required` must remain behind a documented blocker until lawful hands-on evidence confirms the exact native behavior.
...
- Do not copy Gaia GPS branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, route algorithms, vehicle integrations, or protected media.
...
|---|---|---|---|
| Apple App Store listing | https://apps.apple.com/us/app/gaia-gps-offline-trail-maps/id1201979492 | iOS listing, category, age rating, privacy labels, release notes, support links | Public source verified |
| Google Play listing | https://play.google.com/store/apps/details?id=com.trailbehind.android.gaiagps.pro | Android listing, content rating, data safety, feature blurbs, update cadence | Public source verified |
| Official product site | https://www.gaiagps.com/ | Public product orientation for topographic map discovery, route planning, saved waypoints, offline maps, GPS recording, weather/layers, subscription access, sharing, and location privacy | Public source verified |
| Help/support center | https://help.gaiagps.com/ | Support taxonomy, account, payment, cancellation, session, safety, dispute, and recovery flows | Public source verified |
| Privacy policy | https://www.gaiagps.com/privacy/ | Personal data, location, payment, analytics, retention, export, deletion, and regional privacy rights | Public source verified |
| Terms/legal terms | https://www.gaiagps.com/terms/ | Eligibility, acceptable use, payment terms, cancellation, liability, content, and regional service constraints | Public source verified |
...
- Manual verification required: GPS accuracy, offline map download limits, subscription entitlements, third-party map layer licenses, emergency/safety disclaimers, route import/export, background location, and regional map availability.
...
- `ExplorerAccount`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `MapLayer`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `TrailOrRoute`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `Waypoint`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `OfflineRegion`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `TrackRecording`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `SubscriptionEntitlement`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `SharedRoute`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `SafetyNotice`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
- `SupportCase`: stores lifecycle state, ownership, source provenance, authorization rules, sync status, deletion/export treatment, and audit metadata for Gaia GPS-style workflows.
...
- Manual verification checklist tests for GPS accuracy, offline map download limits, subscription entitlements, third-party map layer licenses, emergency/safety disclaimers, route import/export, background location, and regional map availability.
...
- Phase 7: Conduct lawful hands-on verification for GPS accuracy, offline map download limits, subscription entitlements, third-party map layer licenses, emergency/safety disclaimers, route import/export, background location, and regional map availability, then update this spec with evidence before any one-for-one parity claim.
