...
Build an original mobile product inspired by Mimo's public user-facing workflow. The clone focus is: Learner/family onboarding, content or classroom surfaces, progress tracking, messaging, reminders, privacy controls, and minors/student-data gates

This Draft 1 spec reserves ID 711 in the 1000-app backlog. It provides enough structure for downstream research, estimation, and lawful implementation planning, but it is not implementation-ready until exact first-party URLs replace source-discovery links and app-specific public evidence is added.

## Goals

- Deliver a mobile-first education experience with onboarding, primary workflow, settings, support, and recovery flows.
- Reproduce the functional job behind Mimo using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Mimo branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/search?term=Mimo | iOS listing, category, age rating, privacy labels, release notes, support links | Source discovery -- pending exact URL verification |
| Google Play | https://play.google.com/store/search?q=Mimo&c=apps | Android listing, content rating, data safety, feature blurbs | Source discovery -- pending exact URL verification |
| Official website/help search | https://www.google.com/search?q=Mimo%20official%20app%20help%20privacy%20terms | Help center, privacy, terms, support, subscription, safety, and product docs to replace with first-party URLs | Source discovery -- pending exact URL verification |
...
- `User`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Workspace`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Document`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Asset`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Project`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Version`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `ShareGrant`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Template`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `SyncJob`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
- `Notification`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Mimo-style workflows.
