...
Build an original mobile product inspired by Whole Foods Market's public user-facing workflow. The clone focus is: Conversational assistant onboarding, prompt composer, response history, attachments, model/provider routing, safety controls, and subscription gates

This Draft 1 spec reserves ID 400 in the 1000-app backlog. It provides enough structure for downstream research, estimation, and lawful implementation planning, but it is not implementation-ready until exact first-party URLs replace source-discovery links and app-specific public evidence is added.

## Goals

- Deliver a mobile-first grocery and retail experience with onboarding, primary workflow, settings, support, and recovery flows.
- Reproduce the functional job behind Whole Foods Market using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Whole Foods Market branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/search?term=Whole%20Foods%20Market | iOS listing, category, age rating, privacy labels, release notes, support links | Source discovery -- pending exact URL verification |
| Google Play | https://play.google.com/store/search?q=Whole%20Foods%20Market&c=apps | Android listing, content rating, data safety, feature blurbs | Source discovery -- pending exact URL verification |
| Official website/help search | https://www.google.com/search?q=Whole%20Foods%20Market%20official%20app%20help%20privacy%20terms | Help center, privacy, terms, support, subscription, safety, and product docs to replace with first-party URLs | Source discovery -- pending exact URL verification |

## Detailed Design

- Onboarding must support guest, signup, returning-user, permission-primer, and blocked-account states appropriate for grocery and retail.
- Home must default to Browse/Search with empty, loading, personalized, degraded-network, and signed-out variants.
- The primary action must be reachable from Listing/Menu/Product Detail within two taps from home.
- Cart must represent preview, confirmation, or consumption state with saved, shared, unavailable, and error variants.
...
- New user installs, reviews an original value proposition, creates or restores an account, and reaches Browse/Search.
- Returning user opens Browse/Search, resumes the latest meaningful state, and completes the primary action in Listing/Menu/Product Detail.
- User searches or browses, opens Cart, saves or shares the item, and later finds it again from history or library.
...
| Listing/Menu/Product Detail | Primary creation/action flow | taps, forms, deep links | empty, loading, loaded, signed-out | denied permission, offline, stale data, blocked entitlement |
| Cart | Inspect, consume, or confirm details | taps, forms, deep links | empty, loading, loaded, signed-out | denied permission, offline, stale data, blocked entitlement |
| Checkout | Supporting workflow and recovery | taps, forms, deep links | empty, loading, loaded, signed-out | denied permission, offline, stale data, blocked entitlement |
| Order Status | Supporting workflow and recovery | taps, forms, deep links | empty, loading, loaded, signed-out | denied permission, offline, stale data, blocked entitlement |
...
- `User`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Merchant`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `CatalogItem`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Inventory`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Cart`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Order`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `PaymentIntent`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Fulfillment`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Review`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
- `Dispute`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for Whole Foods Market-style workflows.
...
- Reads: GET /users, GET /merchants, GET /catalogitems, GET /inventorys, GET /carts; all reads return authorization status, pagination, cache hints, stale-data markers, and localization keys.
- Writes: POST /users, POST /merchants, POST /catalogitems, POST /inventorys, POST /carts; all writes require validation errors, idempotency keys, optimistic-state reconciliation, and audit events for sensitive state.
...
- Treat payment security as a launch-blocking review area with owner, mitigation, and acceptance tests before implementation.
- Treat consumer protection as a launch-blocking review area with owner, mitigation, and acceptance tests before implementation.
- Treat fraud as a launch-blocking review area with owner, mitigation, and acceptance tests before implementation.
- Treat unsafe or counterfeit goods as a launch-blocking review area with owner, mitigation, and acceptance tests before implementation.
- Treat review abuse as a launch-blocking review area with owner, mitigation, and acceptance tests before implementation.
