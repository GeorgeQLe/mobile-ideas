# Microsoft Azure-Style Clone Spec

> Metadata
> - Inspiration app: Microsoft Azure
...
> - Manual verification blockers: Microsoft Azure account and tenant/subscription policy, owner/contributor/reader RBAC role and custom role model, Microsoft Entra ID (Azure AD) authentication and MFA, virtual machine lifecycle and region/SKU availability, App Service plan scaling and deployment slot management, AKS cluster version and node pool configuration, Azure SQL DTU/vCore and firewall rules, Storage account access tier and SAS token generation, cost management budget and anomaly detection, Azure Monitor action group and alert rule evaluation, Log Analytics workspace and KQL query execution, Entra ID user/group/app registration management, Cloud Shell session provisioning and storage mount, resource tagging and policy compliance, management group and subscription hierarchy, push notification delivery for alert/health/billing events, offline dashboard limitations, ARM API rate limits, and enterprise conditional access policies.
...
Build an original mobile product inspired by Microsoft Azure's public developer tools workflow. The V1 clone focuses on Microsoft Azure mobile app with subscription and resource group overview, virtual machine management (start, stop, restart), App Service monitoring, Azure Kubernetes Service (AKS) cluster status, Azure SQL database management, Storage account browsing, cost management and billing alerts, Azure Monitor alerts and metrics, Log Analytics query, Azure Active Directory user management, Cloud Shell (bash/PowerShell terminal), push notifications for alerts/health/billing, and subscription/tenant-level RBAC.
...
- Preserve the functional job behind Microsoft Azure while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Microsoft Azure branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/microsoft-azure/id1219013620 | iOS listing, privacy labels, release notes, Azure management claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.microsoft.azure | Android listing, data safety, Azure cloud management claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://azure.microsoft.com/ | Microsoft Azure cloud services, infrastructure, AI, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://learn.microsoft.com/en-us/azure/ | Azure documentation, VMs, App Service, AKS, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://privacy.microsoft.com/en-us/privacystatement | Microsoft/Azure account, service data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://azure.microsoft.com/en-us/support/legal/ | Azure services, SLA, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Microsoft Azure account and tenant/subscription policy, owner/contributor/reader RBAC role and custom role model, Microsoft Entra ID (Azure AD) authentication and MFA, virtual machine lifecycle and region/SKU availability, App Service plan scaling and deployment slot management, AKS cluster version and node pool configuration, Azure SQL DTU/vCore and firewall rules, Storage account access tier and SAS token generation, cost management budget and anomaly detection, Azure Monitor action group and alert rule evaluation, Log Analytics workspace and KQL query execution, Entra ID user/group/app registration management, Cloud Shell session provisioning and storage mount, resource tagging and policy compliance, management group and subscription hierarchy, push notification delivery for alert/health/billing events, offline dashboard limitations, ARM API rate limits, and enterprise conditional access policies.

## Core User Journeys

- User creates or restores a Microsoft Azure-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Microsoft Azure.
...
- Phase 4: Add app-specific safety controls for Microsoft Azure mobile app with subscription and resource group overview, virtual machine management (start, stop, restart), App Service monitoring, Azure Kubernetes Service (AKS) cluster status, Azure SQL database management, Storage account browsing, cost management and billing alerts, Azure Monitor alerts and metrics, Log Analytics query, Azure Active Directory user management, Cloud Shell (bash/PowerShell terminal), push notifications for alerts/health/billing, and subscription/tenant-level RBAC.
...
- Confirm legal/privacy retention behavior for Microsoft Azure-style sensitive records and update API contracts before downstream implementation.
