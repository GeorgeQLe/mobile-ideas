# Netlify-Style Clone Spec

> Metadata
> - Inspiration app: Netlify
...
> - Manual verification blockers: Netlify account and team policy, owner/collaborator role and permission model, Git provider connection (GitHub, GitLab, Bitbucket) and build hook configuration, continuous deployment build command and publish directory, deploy preview URL generation and GitHub/GitLab comment integration, instant rollback mechanism and deploy locking, serverless function bundling and invocation limits, edge function Deno runtime and geolocation API, build log streaming and plugin execution, deploy history retention and rollback depth, Netlify DNS zone delegation and record management, environment variable scope (deploy context: production, deploy-preview, branch-deploy), Forms submission handling and spam filtering (Akismet), Identity widget and external provider (Google, GitHub, email) configuration, analytics data collection and pageview counting methodology, split testing branch traffic distribution and cookie behavior, Large Media transformation and Git LFS pointer resolution, team member invitation and site-level access, push notification delivery for deploy/build/form events, offline dashboard limitations, API rate limits, and enterprise SSO/SAML provisioning.
...
Build an original mobile product inspired by Netlify's public developer tools workflow. The V1 clone focuses on web platform with deployment dashboard and site overview, Git-based continuous deployment, deploy previews for branches and PRs, instant rollbacks, serverless functions (Netlify Functions), edge functions, build logs and deploy history, domain and DNS management (Netlify DNS), environment variable and build settings, Forms (serverless form handling), Identity (authentication), analytics, split testing (A/B branch deploys), Large Media (Git LFS), team collaboration and access control, push notifications for deploy-status/build-failures/form-submissions, and plan-gated feature enforcement (Starter, Pro, Business, Enterprise).
...
- Preserve the functional job behind Netlify while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Netlify branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/netlify/id1583393764 | iOS listing, privacy labels, release notes, deployment/hosting claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.netlify.app | Android listing, data safety, deployment/site management claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://www.netlify.com/ | Netlify web platform, deployments, functions, edge, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://docs.netlify.com/ | Netlify documentation, deploys, functions, forms, identity, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://www.netlify.com/privacy/ | Netlify account, deployment data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.netlify.com/legal/terms-of-use/ | Netlify services, subscriptions, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Netlify account and team policy, owner/collaborator role and permission model, Git provider connection (GitHub, GitLab, Bitbucket) and build hook configuration, continuous deployment build command and publish directory, deploy preview URL generation and GitHub/GitLab comment integration, instant rollback mechanism and deploy locking, serverless function bundling and invocation limits, edge function Deno runtime and geolocation API, build log streaming and plugin execution, deploy history retention and rollback depth, Netlify DNS zone delegation and record management, environment variable scope (deploy context: production, deploy-preview, branch-deploy), Forms submission handling and spam filtering (Akismet), Identity widget and external provider (Google, GitHub, email) configuration, analytics data collection and pageview counting methodology, split testing branch traffic distribution and cookie behavior, Large Media transformation and Git LFS pointer resolution, team member invitation and site-level access, push notification delivery for deploy/build/form events, offline dashboard limitations, API rate limits, and enterprise SSO/SAML provisioning.

## Core User Journeys

- User creates or restores a Netlify-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Netlify.
...
- Manual verification tests for Netlify account and team policy, owner/collaborator role and permission model, Git provider connection (GitHub, GitLab, Bitbucket) and build hook configuration, continuous deployment build command and publish directory, deploy preview URL generation and GitHub/GitLab comment integration, instant rollback mechanism and deploy locking, serverless function bundling and invocation limits, edge function Deno runtime and geolocation API, build log streaming and plugin execution, deploy history retention and rollback depth, Netlify DNS zone delegation and record management, environment variable scope (deploy context: production, deploy-preview, branch-deploy), Forms submission handling and spam filtering (Akismet), Identity widget and external provider (Google, GitHub, email) configuration, analytics data collection and pageview counting methodology, split testing branch traffic distribution and cookie behavior, Large Media transformation and Git LFS pointer resolution, team member invitation and site-level access, push notification delivery for deploy/build/form events, offline dashboard limitations, API rate limits, and enterprise SSO/SAML provisioning.
...
- Phase 4: Add app-specific safety controls for web platform with deployment dashboard and site overview, Git-based continuous deployment, deploy previews for branches and PRs, instant rollbacks, serverless functions (Netlify Functions), edge functions, build logs and deploy history, domain and DNS management (Netlify DNS), environment variable and build settings, Forms (serverless form handling), Identity (authentication), analytics, split testing (A/B branch deploys), Large Media (Git LFS), team collaboration and access control, push notifications for deploy-status/build-failures/form-submissions, and plan-gated feature enforcement (Starter, Pro, Business, Enterprise).
...
- Confirm legal/privacy retention behavior for Netlify-style sensitive records and update API contracts before downstream implementation.
