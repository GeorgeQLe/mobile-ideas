# Google Cloud-Style Clone Spec

> Metadata
> - Inspiration app: Google Cloud
...
> - Manual verification blockers: Google Cloud account and project/organization policy, owner/editor/viewer IAM role and custom role model, Google account authentication and OAuth scope, Compute Engine instance lifecycle and zone availability, Cloud Run revision traffic splitting and scaling, GKE cluster version and node auto-repair, Cloud SQL instance connection and maintenance window, Cloud Storage bucket IAM and uniform/fine-grained access, billing account linking and budget threshold configuration, incident notification channel and policy configuration, Cloud Logging query language and log-sink routing, IAM policy binding and conditional access, Cloud Build trigger configuration and build step logs, App Engine version traffic migration, organization policy constraints and folder hierarchy, push notification delivery for incident/billing/alert events, offline dashboard limitations, API quota and rate limits, and Google Workspace/Cloud Identity SSO.
...
Build an original mobile product inspired by Google Cloud's public developer tools workflow. The V1 clone focuses on Google Cloud Platform mobile console with project dashboard and resource overview, Compute Engine VM management, Cloud Run service monitoring, GKE (Kubernetes) cluster status, Cloud SQL instance management, Cloud Storage bucket browsing, billing and budget alerts, incident and uptime monitoring, Cloud Logging log viewer, IAM member and role management, Cloud Build status, App Engine version management, push notifications for incidents/billing/alerts, and project/organization-level access control.
...
- Preserve the functional job behind Google Cloud while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Google Cloud branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/google-cloud/id1005120814 | iOS listing, privacy labels, release notes, GCP management claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.google.android.apps.cloudconsole | Android listing, data safety, GCP cloud management claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://cloud.google.com/ | Google Cloud Platform services, infrastructure, AI/ML, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://cloud.google.com/docs/ | Google Cloud documentation, Compute, Storage, IAM, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://cloud.google.com/terms/cloud-privacy-notice | Google Cloud account, service data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://cloud.google.com/terms/ | Google Cloud services, SLA, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Google Cloud account and project/organization policy, owner/editor/viewer IAM role and custom role model, Google account authentication and OAuth scope, Compute Engine instance lifecycle and zone availability, Cloud Run revision traffic splitting and scaling, GKE cluster version and node auto-repair, Cloud SQL instance connection and maintenance window, Cloud Storage bucket IAM and uniform/fine-grained access, billing account linking and budget threshold configuration, incident notification channel and policy configuration, Cloud Logging query language and log-sink routing, IAM policy binding and conditional access, Cloud Build trigger configuration and build step logs, App Engine version traffic migration, organization policy constraints and folder hierarchy, push notification delivery for incident/billing/alert events, offline dashboard limitations, API quota and rate limits, and Google Workspace/Cloud Identity SSO.

## Core User Journeys

- User creates or restores a Google Cloud-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Google Cloud.
...
- Manual verification tests for Google Cloud account and project/organization policy, owner/editor/viewer IAM role and custom role model, Google account authentication and OAuth scope, Compute Engine instance lifecycle and zone availability, Cloud Run revision traffic splitting and scaling, GKE cluster version and node auto-repair, Cloud SQL instance connection and maintenance window, Cloud Storage bucket IAM and uniform/fine-grained access, billing account linking and budget threshold configuration, incident notification channel and policy configuration, Cloud Logging query language and log-sink routing, IAM policy binding and conditional access, Cloud Build trigger configuration and build step logs, App Engine version traffic migration, organization policy constraints and folder hierarchy, push notification delivery for incident/billing/alert events, offline dashboard limitations, API quota and rate limits, and Google Workspace/Cloud Identity SSO.
...
- Phase 4: Add app-specific safety controls for Google Cloud Platform mobile console with project dashboard and resource overview, Compute Engine VM management, Cloud Run service monitoring, GKE (Kubernetes) cluster status, Cloud SQL instance management, Cloud Storage bucket browsing, billing and budget alerts, incident and uptime monitoring, Cloud Logging log viewer, IAM member and role management, Cloud Build status, App Engine version management, push notifications for incidents/billing/alerts, and project/organization-level access control.
...
- Confirm legal/privacy retention behavior for Google Cloud-style sensitive records and update API contracts before downstream implementation.
