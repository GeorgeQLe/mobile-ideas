# GitLab-Style Clone Spec

> Metadata
> - Inspiration app: GitLab
...
> - Manual verification blockers: GitLab instance (SaaS or self-managed) and namespace policy, guest/reporter/developer/maintainer/owner role and permission model, repository access and clone/pull authentication (SSH, HTTPS, PAT), merge request CRUD and approval rules (required approvers, code owners), CI/CD pipeline trigger rules and runner availability, job log streaming and artifact access, issue CRUD and label/milestone/weight configuration, board column and swimlane configuration, code review thread resolution and suggestion application, branch protection rules and push restrictions, commit GPG signing verification, container registry access and image retention policies, package registry format support and publishing, wiki page editing and versioning, snippet visibility and embedding, to-do list item generation and dismissal, activity feed aggregation and filtering, push notification delivery for pipeline/MR/issue/mention events, offline browsing limitations, self-managed instance connectivity, and SAML/SCIM/SSO provisioning.
...
Build an original mobile product inspired by GitLab's public developer tools workflow. The V1 clone focuses on DevOps platform with repository browsing and file viewer, merge request management (create, review, approve, merge), CI/CD pipeline monitoring and job logs, issue tracking and boards, project and group navigation, code review with inline comments and suggestions, branch management, commit history and diffs, container registry, package registry, wiki, snippets, to-do list, notifications and activity feed, push notifications for pipelines/MRs/mentions, and tier-gated feature enforcement (Free, Premium, Ultimate).
...
- Preserve the functional job behind GitLab while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy GitLab branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/gitlab-devops-in-your-pocket/id1573856457 | iOS listing, privacy labels, release notes, DevOps/repository claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.gitlab.app | Android listing, data safety, DevOps/CI-CD claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://about.gitlab.com/ | GitLab DevOps platform, CI/CD, security, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://docs.gitlab.com/ | GitLab documentation, repository, CI/CD, merge requests, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://about.gitlab.com/privacy/ | GitLab account, repository data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://about.gitlab.com/terms/ | GitLab services, subscriptions, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: GitLab instance (SaaS or self-managed) and namespace policy, guest/reporter/developer/maintainer/owner role and permission model, repository access and clone/pull authentication (SSH, HTTPS, PAT), merge request CRUD and approval rules (required approvers, code owners), CI/CD pipeline trigger rules and runner availability, job log streaming and artifact access, issue CRUD and label/milestone/weight configuration, board column and swimlane configuration, code review thread resolution and suggestion application, branch protection rules and push restrictions, commit GPG signing verification, container registry access and image retention policies, package registry format support and publishing, wiki page editing and versioning, snippet visibility and embedding, to-do list item generation and dismissal, activity feed aggregation and filtering, push notification delivery for pipeline/MR/issue/mention events, offline browsing limitations, self-managed instance connectivity, and SAML/SCIM/SSO provisioning.

## Core User Journeys

- User creates or restores a GitLab-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from GitLab.
...
- Manual verification tests for GitLab instance (SaaS or self-managed) and namespace policy, guest/reporter/developer/maintainer/owner role and permission model, repository access and clone/pull authentication (SSH, HTTPS, PAT), merge request CRUD and approval rules (required approvers, code owners), CI/CD pipeline trigger rules and runner availability, job log streaming and artifact access, issue CRUD and label/milestone/weight configuration, board column and swimlane configuration, code review thread resolution and suggestion application, branch protection rules and push restrictions, commit GPG signing verification, container registry access and image retention policies, package registry format support and publishing, wiki page editing and versioning, snippet visibility and embedding, to-do list item generation and dismissal, activity feed aggregation and filtering, push notification delivery for pipeline/MR/issue/mention events, offline browsing limitations, self-managed instance connectivity, and SAML/SCIM/SSO provisioning.
...
- Phase 4: Add app-specific safety controls for DevOps platform with repository browsing and file viewer, merge request management (create, review, approve, merge), CI/CD pipeline monitoring and job logs, issue tracking and boards, project and group navigation, code review with inline comments and suggestions, branch management, commit history and diffs, container registry, package registry, wiki, snippets, to-do list, notifications and activity feed, push notifications for pipelines/MRs/mentions, and tier-gated feature enforcement (Free, Premium, Ultimate).
...
- Confirm legal/privacy retention behavior for GitLab-style sensitive records and update API contracts before downstream implementation.
