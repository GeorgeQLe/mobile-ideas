# Hacker News-Style Clone Spec

> Metadata
> - Inspiration app: Hacker News
...
> - Manual verification blockers: Hacker News account and karma policy, new-account submission rate limits, story submission URL validation and duplicate detection, upvote/downvote karma threshold requirements, comment depth limits and rendering, Show HN and Ask HN formatting conventions, ranking algorithm opacity and flag/penalty behavior, moderator intervention and story editing, saved stories implementation (account-linked), job posting format and monthly cycle (Who is Hiring), poll creation restrictions, Algolia search index freshness and query limits, RSS feed generation and item limits, UI rendering constraints (minimal HTML, no JavaScript framework), third-party client API (Firebase-based) rate limits and data model, push notification delivery (third-party client implementation), offline story caching, and community moderation guidelines (on-topic, civil, substantive).
...
Build an original mobile product inspired by Hacker News's public developer tools workflow. The V1 clone focuses on technology news aggregator with link submission and voting, Show HN (project showcases), Ask HN (community questions), comment threads with nested replies, karma and user profiles, story ranking algorithm (points, age, penalties), flagging and moderation, saved stories and favorites, job postings (Who is Hiring), polls, search (via Algolia), RSS feeds, minimal UI with text-focused design, push notifications for replies/mentions (via third-party clients), and community-governed moderation (dang, moderator guidelines).
...
- Preserve the functional job behind Hacker News while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Hacker News branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/hacker-news/id1334211500 | iOS listing (third-party clients), privacy labels, release notes, and HN reader claims | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=io.github.hidroh.materialistic | Android listing (third-party clients), data safety, HN reader claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://news.ycombinator.com/ | Hacker News front page, submission guidelines, and community positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://news.ycombinator.com/newsguidelines.html | Hacker News guidelines, moderation rules, and community standards | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://www.ycombinator.com/legal#privacy | Y Combinator/HN account data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.ycombinator.com/legal | Y Combinator/HN services and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Hacker News account and karma policy, new-account submission rate limits, story submission URL validation and duplicate detection, upvote/downvote karma threshold requirements, comment depth limits and rendering, Show HN and Ask HN formatting conventions, ranking algorithm opacity and flag/penalty behavior, moderator intervention and story editing, saved stories implementation (account-linked), job posting format and monthly cycle (Who is Hiring), poll creation restrictions, Algolia search index freshness and query limits, RSS feed generation and item limits, UI rendering constraints (minimal HTML, no JavaScript framework), third-party client API (Firebase-based) rate limits and data model, push notification delivery (third-party client implementation), offline story caching, and community moderation guidelines (on-topic, civil, substantive).

## Core User Journeys

- User creates or restores a Hacker News-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Hacker News.
...
- Manual verification tests for Hacker News account and karma policy, new-account submission rate limits, story submission URL validation and duplicate detection, upvote/downvote karma threshold requirements, comment depth limits and rendering, Show HN and Ask HN formatting conventions, ranking algorithm opacity and flag/penalty behavior, moderator intervention and story editing, saved stories implementation (account-linked), job posting format and monthly cycle (Who is Hiring), poll creation restrictions, Algolia search index freshness and query limits, RSS feed generation and item limits, UI rendering constraints (minimal HTML, no JavaScript framework), third-party client API (Firebase-based) rate limits and data model, push notification delivery (third-party client implementation), offline story caching, and community moderation guidelines (on-topic, civil, substantive).
...
- Confirm legal/privacy retention behavior for Hacker News-style sensitive records and update API contracts before downstream implementation.
