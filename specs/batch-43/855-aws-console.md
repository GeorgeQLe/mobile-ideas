# AWS Console-Style Clone Spec

> Metadata
> - Inspiration app: AWS Console
...
> - Manual verification blockers: AWS account and IAM policy, root/IAM-user/IAM-role/federated identity access model, MFA device enrollment and session token handling, EC2 instance lifecycle and region-specific availability, CloudWatch metric namespace and custom metric publishing, CloudWatch alarm evaluation period and action configuration, S3 bucket policy and CORS/access-point configuration, billing and cost explorer data granularity and linked-account access, AWS Health event filtering and affected-resource correlation, CloudFormation stack event streaming and rollback behavior, Lambda function invocation logs and cold-start monitoring, DynamoDB table scan/query and capacity mode, Route 53 hosted zone and record set management, IAM policy evaluation and permission boundary complexity, AWS Organizations and SCP (service control policy) inheritance, push notification delivery for alarm/health/billing events, offline dashboard limitations, STS token expiry and refresh, and AWS SSO/Identity Center provisioning.
...
Build an original mobile product inspired by AWS Console's public developer tools workflow. The V1 clone focuses on Amazon Web Services mobile console with service dashboard and resource overview, EC2 instance management (start, stop, reboot), CloudWatch alarms and metrics, S3 bucket browsing, billing and cost explorer, AWS Health dashboard and event notifications, CloudFormation stack status, Lambda function monitoring, DynamoDB table browsing, Route 53 DNS management, IAM user and role overview, push notifications for alarms/health-events/billing-alerts, and account/organization-level access control.
...
- Preserve the functional job behind AWS Console while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy AWS Console branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/aws-console/id580990573 | iOS listing, privacy labels, release notes, AWS management claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.amazon.aws.console.mobile | Android listing, data safety, AWS cloud management claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://aws.amazon.com/console/mobile/ | AWS Console Mobile App features, services, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://docs.aws.amazon.com/consolemobileapp/latest/userguide/ | AWS Console Mobile documentation, setup, services, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://aws.amazon.com/privacy/ | AWS/Amazon account, service data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://aws.amazon.com/service-terms/ | AWS services, SLA, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: AWS account and IAM policy, root/IAM-user/IAM-role/federated identity access model, MFA device enrollment and session token handling, EC2 instance lifecycle and region-specific availability, CloudWatch metric namespace and custom metric publishing, CloudWatch alarm evaluation period and action configuration, S3 bucket policy and CORS/access-point configuration, billing and cost explorer data granularity and linked-account access, AWS Health event filtering and affected-resource correlation, CloudFormation stack event streaming and rollback behavior, Lambda function invocation logs and cold-start monitoring, DynamoDB table scan/query and capacity mode, Route 53 hosted zone and record set management, IAM policy evaluation and permission boundary complexity, AWS Organizations and SCP (service control policy) inheritance, push notification delivery for alarm/health/billing events, offline dashboard limitations, STS token expiry and refresh, and AWS SSO/Identity Center provisioning.

## Core User Journeys

- User creates or restores a AWS Console-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from AWS Console.
...
- Manual verification tests for AWS account and IAM policy, root/IAM-user/IAM-role/federated identity access model, MFA device enrollment and session token handling, EC2 instance lifecycle and region-specific availability, CloudWatch metric namespace and custom metric publishing, CloudWatch alarm evaluation period and action configuration, S3 bucket policy and CORS/access-point configuration, billing and cost explorer data granularity and linked-account access, AWS Health event filtering and affected-resource correlation, CloudFormation stack event streaming and rollback behavior, Lambda function invocation logs and cold-start monitoring, DynamoDB table scan/query and capacity mode, Route 53 hosted zone and record set management, IAM policy evaluation and permission boundary complexity, AWS Organizations and SCP (service control policy) inheritance, push notification delivery for alarm/health/billing events, offline dashboard limitations, STS token expiry and refresh, and AWS SSO/Identity Center provisioning.
...
- Phase 4: Add app-specific safety controls for Amazon Web Services mobile console with service dashboard and resource overview, EC2 instance management (start, stop, reboot), CloudWatch alarms and metrics, S3 bucket browsing, billing and cost explorer, AWS Health dashboard and event notifications, CloudFormation stack status, Lambda function monitoring, DynamoDB table browsing, Route 53 DNS management, IAM user and role overview, push notifications for alarms/health-events/billing-alerts, and account/organization-level access control.
...
- Confirm legal/privacy retention behavior for AWS Console-style sensitive records and update API contracts before downstream implementation.
