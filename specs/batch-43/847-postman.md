# Postman-Style Clone Spec

> Metadata
> - Inspiration app: Postman
...
> - Manual verification blockers: Postman account and team workspace policy, viewer/editor/admin role and permission model, request builder protocol support and payload validation, collection CRUD and folder organization, runner execution and iteration data handling, environment variable scoping (global, collection, environment, local), API documentation hosting and custom domain, mock server response matching and delay simulation, monitor scheduling and notification thresholds, team workspace member management and sharing, version control fork/merge conflict resolution, pre-request and test script execution sandbox, response body rendering and format detection, cookie jar management and domain scoping, client certificate and CA configuration, Flows canvas rendering and step execution, API governance rule evaluation and linting, push notification delivery for monitor failures/team mentions/workspace events, offline collection access, workspace sync and conflict resolution, and enterprise SSO/SCIM provisioning.
...
Build an original mobile product inspired by Postman's public developer tools workflow. The V1 clone focuses on API development platform with request builder (REST, GraphQL, WebSocket, gRPC), collection organization and runner, environment and variable management, API documentation generation, mock servers, monitors for scheduled testing, team workspaces and collaboration, version control with forking and merging, pre-request and test scripts (JavaScript), response visualization, cookie manager, certificate management, Postman Flows (visual API workflow builder), API governance and design (OpenAPI, AsyncAPI), push notifications for monitors/collaboration/mentions, and plan-gated feature enforcement (Free, Basic, Professional, Enterprise).
...
- Preserve the functional job behind Postman while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Postman branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/postman-api-platform/id1607621948 | iOS listing, privacy labels, release notes, API development claims, and support links | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.postman.app | Android listing, data safety, API platform/collaboration claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official product site | https://www.postman.com/ | Postman API platform, collections, workspaces, and product positioning | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Official help | https://learning.postman.com/docs/ | Postman documentation, requests, collections, environments, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Privacy policy | https://www.postman.com/legal/privacy-policy/ | Postman account, API data, workspace data, analytics, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
| Terms | https://www.postman.com/legal/terms/ | Postman services, subscriptions, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-05; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Postman account and team workspace policy, viewer/editor/admin role and permission model, request builder protocol support and payload validation, collection CRUD and folder organization, runner execution and iteration data handling, environment variable scoping (global, collection, environment, local), API documentation hosting and custom domain, mock server response matching and delay simulation, monitor scheduling and notification thresholds, team workspace member management and sharing, version control fork/merge conflict resolution, pre-request and test script execution sandbox, response body rendering and format detection, cookie jar management and domain scoping, client certificate and CA configuration, Flows canvas rendering and step execution, API governance rule evaluation and linting, push notification delivery for monitor failures/team mentions/workspace events, offline collection access, workspace sync and conflict resolution, and enterprise SSO/SCIM provisioning.

## Core User Journeys

- User creates or restores a Postman-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
|---|---|---|---|---|
| Welcome/Auth | Account login, workspace/instance selection, SSO/SAML, token-based auth, and permission primers | email, SSO, token, instance-select, biometric | new, returning, multi-instance, locked | SSO failed, instance unreachable, token expired, auth timeout |
| Repository/Project | Repository list, project navigation, file browser, README viewer, branch selector, and clone/fork actions | browse, search, select-branch, fork, star | loading, loaded, empty, forked | repo unavailable, branch missing, permission denied, sync failed |
| Merge/Pull Request | MR/PR list, diff viewer, inline comments, review threads, approval status, and merge actions | review, comment, approve, merge, request-changes | open, reviewing, approved, merged, closed, conflicts | merge conflict, CI failed, approval required, force-push detected |
| CI/CD Pipelines | Pipeline list, job status, build logs, artifact downloads, and manual trigger/retry actions | view, trigger, retry, cancel, download | running, passed, failed, canceled, pending | runner unavailable, timeout, artifact expired, quota exceeded |
| Issues/Boards | Issue list, kanban board, issue detail, labels, assignees, milestones, and linked MRs/PRs | create, assign, label, move, close, link | open, in-progress, closed, blocked | permission denied, milestone overdue, duplicate detected, board sync failed |
| Settings/Privacy | Account, workspace, tokens, notification preferences, SSH keys, privacy, support, and data export | forms, toggle, configure, generate, request | loaded, editing, syncing | token revoked, key invalid, permission denied, export pending |
...
- Treat source code and repository contents, merge/pull request discussions and review comments, CI/CD pipeline configurations and secrets, SSH keys and access tokens, environment variables and deployment credentials, container registry images, issue tracker content and internal discussions, team member permissions and access control lists, API keys and webhook payloads, build artifacts and logs, workspace and project configuration, and enterprise SSO/SCIM/SAML credentials as launch-blocking review areas with owners, mitigations, and acceptance tests before implementation.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Postman.
...
- Manual verification tests for Postman account and team workspace policy, viewer/editor/admin role and permission model, request builder protocol support and payload validation, collection CRUD and folder organization, runner execution and iteration data handling, environment variable scoping (global, collection, environment, local), API documentation hosting and custom domain, mock server response matching and delay simulation, monitor scheduling and notification thresholds, team workspace member management and sharing, version control fork/merge conflict resolution, pre-request and test script execution sandbox, response body rendering and format detection, cookie jar management and domain scoping, client certificate and CA configuration, Flows canvas rendering and step execution, API governance rule evaluation and linting, push notification delivery for monitor failures/team mentions/workspace events, offline collection access, workspace sync and conflict resolution, and enterprise SSO/SCIM provisioning.
...
- Phase 4: Add app-specific safety controls for API development platform with request builder (REST, GraphQL, WebSocket, gRPC), collection organization and runner, environment and variable management, API documentation generation, mock servers, monitors for scheduled testing, team workspaces and collaboration, version control with forking and merging, pre-request and test scripts (JavaScript), response visualization, cookie manager, certificate management, Postman Flows (visual API workflow builder), API governance and design (OpenAPI, AsyncAPI), push notifications for monitors/collaboration/mentions, and plan-gated feature enforcement (Free, Basic, Professional, Enterprise).
...
- Confirm legal/privacy retention behavior for Postman-style sensitive records and update API contracts before downstream implementation.
