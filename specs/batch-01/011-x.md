...
Build an original mobile product inspired by X's public workflow: real-time public conversation, posts, reposts, replies, DMs, trends, live audio/video, creator subscriptions, premium verification-style entitlements, ads, safety rules, data archive, and account deactivation.

The clone must not copy X branding, screenshots, marketing copy, protected UI artwork, ranking systems, private APIs, proprietary data, marketplace assets, moderation tooling, or paid-plan names. The implementation can reproduce comparable user jobs and interaction patterns with original product language and licensed or first-party providers.
...
- Do not build a X-branded app or imply affiliation with X or its owner.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/x/id333903271 | Official iOS listing, category, age rating, supported devices, privacy labels, in-app purchases, version notes, and support/privacy links | Verified 2026-04-17 |
| Google Play | https://play.google.com/store/apps/details?id=com.twitter.android | Official Android listing, package id, content rating, downloads, data safety, in-app purchases/ads, support contact, and public feature claims | Verified 2026-04-17 |
| X Help Center | https://help.x.com/ | Public support entry point for accounts, posting, DMs, privacy, safety, and monetization | Verified 2026-04-17 |
| X Privacy Help | https://help.x.com/en/privacy | Privacy and data support route | Verified 2026-04-17 |
| X Privacy Policy | https://x.com/en/privacy | Personal data collection, use, sharing, data archive, deactivation, deletion, and rights | Verified 2026-04-17 |
| X Terms | https://x.com/en/tos | Service terms, content rights, account termination, and deactivation reference | Verified 2026-04-17 |
| X Rules And Policies | https://help.x.com/rules-and-policies | Platform rules, policy enforcement, and safety support | Verified 2026-04-17 |
| Deactivate X Account Help | https://help.x.com/managing-your-account/how-to-deactivate-x-account | Account deactivation and deletion path | Verified 2026-04-17 |

## Detailed Design

### Source-Backed Product Requirements

- The public listings position X as a real-time public conversation app for breaking news, posts, photos/videos, communities, trends, DMs, live events, podcasts/video, ads, premium subscriptions, creator subscriptions, and in-app purchases.
- Timeline ranking must support following, recommended, latest, trends, media, muted words, blocked accounts, sensitive content, degraded service, and unavailable post states.
- Posts must support text, media, polls where included, replies, reposts, quote-style sharing, likes, bookmarks, views, edit where entitled, delete, report, and visibility controls.
- DMs must separate public identity from private messages, support requests, media, blocking, reporting, encryption claims only if independently implemented, and privacy-safe push payloads.
- Premium and creator monetization must be original, server-authoritative, and tested for paid features, app-store/web ownership, restore, refund, expiration, creator payouts, and ads.
- Data archive and account deactivation/deletion must be exposed from settings with clear deletion queue state and reactivation consequences.
- Safety controls must cover impersonation, harassment, hateful conduct, violent content, adult content, synthetic media, spam, platform manipulation, and crisis/regulated content.
- Manual verification remains required for exact native navigation, premium verification states, edit limits, live spaces/video, push payloads, data archive, and policy labels.
...
| Home Timeline | Home Timeline workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Communities | Communities workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Live Audio/Video | Live Audio/Video workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Creator Dashboard | Creator Dashboard workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
- - `Profile`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Post`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `MediaAsset`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Reaction`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Repost`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Follow`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `DirectThread`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Trend`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Community`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Entitlement`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `ModerationCase`: X-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
...
- Treat real-time abuse as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat misinformation as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat adult content as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat impersonation as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat creator monetization as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat public data retention as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
...
- A downstream team can build the V1 without proprietary X assets, private APIs, network traffic, brand copy, ranking systems, or moderation tooling.
