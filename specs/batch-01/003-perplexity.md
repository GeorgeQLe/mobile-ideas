# Perplexity-Style Clone Spec

> Metadata
> - Inspiration app: Perplexity
> - Category: AI search
...
Build an original mobile product inspired by Perplexity's public workflow: answer-engine workflows with cited search results, follow-up threads, discover/library surfaces, voice search, file/image prompts, assistant actions, subscription limits, and data deletion controls.

The clone must not copy Perplexity branding, screenshots, marketing copy, protected UI artwork, ranking systems, private APIs, proprietary data, marketplace assets, moderation tooling, or paid-plan names. The implementation can reproduce comparable user jobs and interaction patterns with original product language and licensed or first-party providers.
...
- Do not build a Perplexity-branded app or imply affiliation with Perplexity or its owner.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/perplexity-ask-anything/id1668000334 | Official iOS listing, category, age rating, supported devices, privacy labels, in-app purchases, version notes, and support/privacy links | Verified 2026-04-17 |
| Google Play | https://play.google.com/store/apps/details?id=ai.perplexity.app.android | Official Android listing, package id, content rating, downloads, data safety, in-app purchases/ads, support contact, and public feature claims | Verified 2026-04-17 |
| Perplexity Help Center | https://www.perplexity.ai/help-center/en/ | Public support entry point for account, subscriptions, app behavior, library, search, and troubleshooting | Verified 2026-04-17 |
| Self-Serve Data Deletion | https://www.perplexity.ai/help-center/en/articles/11564562-self-serve-data-deletion | Account deletion and specific data deletion behavior | Verified 2026-04-17 |
| Account Deletion | https://www.perplexity.ai/help-center/en/articles/10354879-account-deletion | Thirty-day account deletion window, deleted content behavior, and GDPR request path | Verified 2026-04-17 |
| Perplexity Terms | https://www.perplexity.ai/hub/legal/terms-of-service | Account, usage, subscriptions, acceptable use, and service terms | Verified 2026-04-17 |
| Perplexity Privacy Policy | https://www.perplexity.ai/hub/legal/privacy-policy | Personal data categories, usage, retention, sharing, and privacy rights | Verified 2026-04-17 |
| Perplexity Pro | https://www.perplexity.ai/pro | Public subscription positioning for paid answer/search capabilities | Verified 2026-04-17 |

## Detailed Design

### Source-Backed Product Requirements

- The public listings position Perplexity as a cited AI search and discovery app with Pro Search, Deep Research, follow-up threads, voice input, Discover, Library, and paid AI-model access.
- Every answer must carry source/citation metadata, retrieval timestamps, query mode, model/provider attribution class, and confidence or source-coverage state.
- Search threads must support follow-up questions, source expansion, copy/share, save to library, delete, and safe fallback when retrieval or generation fails.
- Discover and Library surfaces must separate public/community discovery from private history and saved research objects.
- Assistant/action features such as scheduling, drafting, bookings, or file creation must ship as opt-in integrations with provider-specific permission grants.
- Account deletion and data deletion must expose a self-serve path, cancellation warning for subscriptions, and delayed irreversible deletion state.
- Subscription states must distinguish free, pro-style, max-style, app-store-managed, web-managed, trial, expired, canceled, refunded, and unavailable states without copying plan names or pricing.
- Manual verification remains required for exact mobile navigation, voice search, app-store purchase restore, push notifications, assistant integrations, and region-specific model availability.
...
| Ask Home | Ask Home workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Answer Thread | Answer Thread workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Source Detail | Source Detail workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Library | Library workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Voice Search | Voice Search workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Attachment Prompt | Attachment Prompt workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Assistant Actions | Assistant Actions workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Account And Data | Account And Data workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
- - `SearchThread`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Answer`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Citation`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `SourceDocument`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `LibraryItem`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `AssistantAction`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Upload`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Entitlement`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `DataDeletionRequest`: Perplexity-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
...
- Treat citation quality as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat AI hallucination as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat source licensing as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat assistant action consent as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat search history privacy as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat paid model limits as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat high-stakes advice as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
...
- A downstream team can build the V1 without proprietary Perplexity assets, private APIs, network traffic, brand copy, ranking systems, or moderation tooling.
