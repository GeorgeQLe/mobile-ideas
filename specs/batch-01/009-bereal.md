# BeReal-Style Clone Spec

> Metadata
> - Inspiration app: BeReal
...
Build an original mobile product inspired by BeReal's public workflow: once-daily dual-camera posting, timed notifications, friends feed, public discovery, comments, RealMoji-style reactions, maps, memories, recaps, account deletion, and authenticity-focused safety controls.

The clone must not copy BeReal branding, screenshots, marketing copy, protected UI artwork, ranking systems, private APIs, proprietary data, marketplace assets, moderation tooling, or paid-plan names. The implementation can reproduce comparable user jobs and interaction patterns with original product language and licensed or first-party providers.
...
- Do not build a BeReal-branded app or imply affiliation with BeReal or its owner.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/bereal-photos-friends-daily/id1459645446 | Official iOS listing, category, age rating, supported devices, privacy labels, in-app purchases, version notes, and support/privacy links | Verified 2026-04-17 |
| Google Play | https://play.google.com/store/apps/details?id=com.bereal.ft | Official Android listing, package id, content rating, downloads, data safety, in-app purchases/ads, support contact, and public feature claims | Verified 2026-04-17 |
| BeReal Help Center | https://help.bereal.com/hc/en-us | Public support entry point for account, posting, friends, privacy, and troubleshooting | Verified 2026-04-17 |
| Delete Your Account | https://help.bereal.com/hc/en-us/articles/7531255012381-Delete-your-Account | In-app deletion path and 15-day deletion window | Verified 2026-04-17 |
| BeReal Terms | https://bereal.com/terms/ | Terms, account deletion, three-strike policy, inactivity deletion, and suspension behavior | Verified 2026-04-17 |
| BeReal Privacy Policy | https://bereal.com/privacy-2/ | Personal data categories, age scope, controls, location, and sharing | Verified 2026-04-17 |
| BeReal Community Standards | https://bereal.com/community-standards | Authenticity, safety, and violative content rules | Verified 2026-04-17 |
| BeReal Contact | https://bereal.com/contact | Public support and contact route | Verified 2026-04-17 |

## Detailed Design

### Source-Backed Product Requirements

- The public listings position BeReal as a once-a-day photo-sharing app with random timed prompts, two-minute capture, dual-camera photos, friends, discovery, comments, reactions, map, memories, and recap-style content.
- The V1 must model daily prompt windows, late posts, missed prompts, retakes, dual-camera capture, caption, audience, public discovery, and blocked posting states.
- Friend feed and discovery feed must separate friend-only, public, location-aware, unavailable, reported, deleted, and expired content states.
- RealMoji-style reactions must be original assets and must support capture permission, reuse, delete, visibility, and moderation state.
- Map and location features must be opt-in, audience-scoped, coarse/precise-aware, revocable, and disabled by default for users who do not grant location.
- Memories and recaps must clearly distinguish private archives from friend-visible content and include delete/export paths.
- Account deletion must support an in-app path, scheduled deletion state, cancellation by re-login inside the documented window, and support request fallback.
- Manual verification remains required for exact prompt timing behavior, retake indicators, dual-camera capture, map visibility, recap behavior, push payloads, and region/age availability.
...
| Daily Prompt | Daily Prompt workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Dual Camera Capture | Dual Camera Capture workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Friends Feed | Friends Feed workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Discovery | Discovery workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Comments And Reactions | Comments And Reactions workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
| Recap | Recap workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
| Settings | Settings workflow and recovery states | taps, text, media, permissions, deep links | empty, loading, loaded, signed-out, restricted | denied permission, offline, stale data, blocked entitlement, unavailable content |
...
- - `DailyPrompt`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `BeRealPost`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `MediaPair`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Reaction`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Comment`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `Friendship`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `LocationShare`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `MemoryArchive`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `DeletionRequest`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
- - `ModerationCase`: BeReal-specific lifecycle, ownership, authorization, sync, visibility, moderation, and deletion/export metadata.
...
- Treat location privacy as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat push timing as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat authenticity pressure as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat public discovery moderation as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat photo retention as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
- Treat account deletion timing as a launch-blocking review area with owner, mitigation, test coverage, and manual-verification status.
...
- A downstream team can build the V1 without proprietary BeReal assets, private APIs, network traffic, brand copy, ranking systems, or moderation tooling.
