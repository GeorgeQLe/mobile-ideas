# Shipt-Style Clone Spec

> Metadata
> - Inspiration app: Shipt
...
Build an original mobile product inspired by Shipt's public product and policy materials. V1 focuses on shopper/driver onboarding, eligibility checks, order offers, grocery picking, member messaging, substitutions, delivery windows, earnings, same-day pay, support, and safety. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support shopper/driver onboarding, eligibility checks, order offers, grocery picking, member messaging, substitutions, delivery windows, earnings, same-day pay, support, and safety with clear retry, recovery, unavailable, region, entitlement, worker, and provider-aware states.
...
- Do not imply affiliation with Shipt or its publisher.
...
|---|---|---|---|
| Official product | https://www.shipt.com/be-a-shopper/ | Public product positioning, service categories, ordering, worker, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.shipt.com/ | Public support taxonomy for account, order/activity, billing/payment, worker eligibility, provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.shipt.com/privacy-policy/ | Account, profile, order/work activity, location, device, advertising, analytics, support, worker, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.shipt.com/terms-of-service/ | Service usage, licensed content, payments, user/worker conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or worker program | https://www.shipt.com/be-a-shopper/ | Reward, membership, loyalty, worker, payout, offer, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/shipt-shopper-and-driver/id976353472 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.shipt.shopper | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- shopper/driver onboarding, eligibility checks, order offers, grocery picking, member messaging, substitutions, delivery windows, earnings, same-day pay, support, and safety must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for Shipt-inspired behavior | taps, forms, search, filters, deep links, location, payment, identity, merchant/customer/courier selection | empty, loading, loaded, signed-out, offline, entitlement-aware, region-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, merchant closed, item unavailable, worker ineligible, payment/payout failed |
...
- `Worker`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `EligibilityProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `RegionZone`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `VehicleDocument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `AvailabilityState`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `Task`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `MerchantPickup`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `CustomerDropoff`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `RouteLeg`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `EarningsLedger`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `PayoutInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `SafetyIncident`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `AppealCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for Shipt-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from Shipt.
