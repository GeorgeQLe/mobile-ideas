# DoorDash Dasher-Style Clone Spec

> Metadata
> - Inspiration app: DoorDash Dasher
...
Build an original mobile product inspired by DoorDash Dasher's public product and policy materials. V1 focuses on Dasher signup, identity/background checks, schedule or Dash Now, offer acceptance, merchant pickup, customer dropoff, tips/earnings, payout products, support, and safety incidents. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support Dasher signup, identity/background checks, schedule or Dash Now, offer acceptance, merchant pickup, customer dropoff, tips/earnings, payout products, support, and safety incidents with clear retry, recovery, unavailable, region, entitlement, worker, and provider-aware states.
- Preserve boundaries between worker, identity, eligibility, task, merchant, customer, route, location, earnings, payout, safety, support, advertising, and marketplace data.
...
- Do not imply affiliation with DoorDash Dasher or its publisher.
...
|---|---|---|---|
| Official product | https://dasher.doordash.com/ | Public product positioning, service categories, ordering, worker, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://help.doordash.com/dashers/s/ | Public support taxonomy for account, order/activity, billing/payment, worker eligibility, provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://help.doordash.com/legal/document?type=dx-privacy-policy | Account, profile, order/work activity, location, device, advertising, analytics, support, worker, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://help.doordash.com/legal/document?type=dx-ica | Service usage, licensed content, payments, user/worker conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or worker program | https://dasher.doordash.com/en-us/dasherdirect | Reward, membership, loyalty, worker, payout, offer, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/us/app/doordash-dasher/id1451754591 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.doordash.driverapp | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- Dasher signup, identity/background checks, schedule or Dash Now, offer acceptance, merchant pickup, customer dropoff, tips/earnings, payout products, support, and safety incidents must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
- worker classification, background/identity checks, right-to-work eligibility, vehicle/insurance checks, offer fairness, route safety, precise-location privacy, customer messaging, earnings/payout correctness, tax documents, deactivation appeals, and incident escalation must be treated as a launch-blocking risk area with owner, mitigation, and acceptance tests.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for DoorDash Dasher-inspired behavior | taps, forms, search, filters, deep links, location, payment, identity, merchant/customer/courier selection | empty, loading, loaded, signed-out, offline, entitlement-aware, region-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, merchant closed, item unavailable, worker ineligible, payment/payout failed |
...
- `Worker`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `EligibilityProfile`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `RegionZone`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `VehicleDocument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `AvailabilityState`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `Offer`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `Task`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `MerchantPickup`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `CustomerDropoff`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `RouteLeg`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `EarningsLedger`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `PayoutInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `SafetyIncident`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `AppealCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for DoorDash Dasher-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from DoorDash Dasher.
