# SkipTheDishes-Style Clone Spec

> Metadata
> - Inspiration app: SkipTheDishes
...
Build an original mobile product inspired by SkipTheDishes's public product and policy materials. V1 focuses on Canada city serviceability, restaurant and retail discovery, cart/checkout, Skip+ benefits, points, order tracking, alcohol/pharmacy-adjacent blockers, support/refunds, and regional availability. The clone must use original branding, original UI copy, original sample data, licensed providers, and explicit disclosures when a feature is inferred from public materials rather than verified hands-on behavior.
...
- Support Canada city serviceability, restaurant and retail discovery, cart/checkout, Skip+ benefits, points, order tracking, alcohol/pharmacy-adjacent blockers, support/refunds, and regional availability with clear retry, recovery, unavailable, region, entitlement, worker, and provider-aware states.
...
- Do not imply affiliation with SkipTheDishes or its publisher.
...
|---|---|---|---|
| Official product | https://www.skipthedishes.com/ | Public product positioning, service categories, ordering, worker, account, location, membership/reward, and app behavior | Verified 2026-05-04 |
| Support/help center | https://www.skipthedishes.com/help | Public support taxonomy for account, order/activity, billing/payment, worker eligibility, provider issues, troubleshooting, and deletion/export flows | Verified 2026-05-04 |
| Privacy policy | https://www.skipthedishes.com/privacy-policy | Account, profile, order/work activity, location, device, advertising, analytics, support, worker, and user-rights handling | Verified 2026-05-04 |
| Terms of service | https://www.skipthedishes.com/terms-of-service | Service usage, licensed content, payments, user/worker conduct, provider limits, regulated-flow boundaries, and legal boundaries | Verified 2026-05-04 |
| Rewards, membership, or worker program | https://www.skipthedishes.com/skipplus | Reward, membership, loyalty, worker, payout, offer, or entitlement orientation where public | Verified 2026-05-04 |
| App Store listing | https://apps.apple.com/ca/app/skip-food-grocery-delivery/id858315554 | Canonical iOS listing, category, age rating, privacy label, compatibility, public feature claims, and native metadata | Verified 2026-05-04 |
| Google Play listing | https://play.google.com/store/apps/details?id=com.ncconsulting.skipthedishes_android | Canonical Android listing, feature blurbs, content rating, data safety, downloads, and native metadata | Verified 2026-05-04 |
...
- Canada city serviceability, restaurant and retail discovery, cart/checkout, Skip+ benefits, points, order tracking, alcohol/pharmacy-adjacent blockers, support/refunds, and regional availability must be treated as launch-blocking behavior with owners, mitigations, acceptance tests, and manual-verification notes.
...
|---|---|---|---|---|
| Welcome / Auth / Consent | Defines the welcome / auth / consent workflow for SkipTheDishes-inspired behavior | taps, forms, search, filters, deep links, location, payment, identity, merchant/customer/courier selection | empty, loading, loaded, signed-out, offline, entitlement-aware, region-aware | permission denied, stale state, entitlement blocked, provider failed, region unavailable, merchant closed, item unavailable, worker ineligible, payment/payout failed |
...
- `User`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `AccountSession`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Address`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Merchant`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `MerchantCatalog`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `MenuItem`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `ModifierGroup`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Cart`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Promotion`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Membership`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `PaymentInstrument`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `Order`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `CourierAssignment`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `DeliveryStatus`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `SupportCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `RefundCase`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `AuditEvent`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
- `LocalCacheRecord`: owner, lifecycle state, authorization boundary, sync state, retention policy, deletion/export behavior, and audit metadata for SkipTheDishes-inspired workflows.
...
- Public API documentation must use original route names and domain models; never mimic private endpoint paths, request bodies, or proprietary provider schemas from SkipTheDishes.
