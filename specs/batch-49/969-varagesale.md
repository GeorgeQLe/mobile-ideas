...
Build an original mobile product inspired by VarageSale's public user-facing workflow. The clone focus is: VarageSale-inspired onboarding, primary workflow, search/detail surfaces, settings, notifications, privacy/safety, monetization, and testable edge cases

This Draft 1 spec reserves ID 969 in the 1000-app backlog. It provides enough structure for downstream research, estimation, and lawful implementation planning, but it is not implementation-ready until exact first-party URLs replace source-discovery links and app-specific public evidence is added.
...
- Reproduce the functional job behind VarageSale using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy VarageSale branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, ranking systems, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/search?term=VarageSale | iOS listing, category, age rating, privacy labels, release notes, support links | Source discovery -- pending exact URL verification |
| Google Play | https://play.google.com/store/search?q=VarageSale&c=apps | Android listing, content rating, data safety, feature blurbs | Source discovery -- pending exact URL verification |
| Official website/help search | https://www.google.com/search?q=VarageSale%20official%20app%20help%20privacy%20terms | Help center, privacy, terms, support, subscription, safety, and product docs to replace with first-party URLs | Source discovery -- pending exact URL verification |
...
- `User`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Workspace`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Document`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Asset`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Project`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Version`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `ShareGrant`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Template`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `SyncJob`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
- `Notification`: stores lifecycle state, authorization boundaries, audit metadata, deletion/export behavior, and sync state for VarageSale-style workflows.
