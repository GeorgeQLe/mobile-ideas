# Clean Email-Style Clone Spec

> Metadata
> - Inspiration app: Clean Email
...
> - Manual verification blockers: Clean Email account and email account linking (Gmail OAuth, Outlook OAuth, Yahoo, iCloud, IMAP), smart views email grouping algorithm (sender, type, date, size clustering), bulk action execution at scale (batch archive, trash, move, label across thousands of emails), Auto Clean rule engine (recurring rule scheduling, condition matching, action execution), Unsubscriber mailing list detection and unsubscribe execution (List-Unsubscribe header, web scraping), Screener new sender detection and blocking/approval flow, Keep Newest rule per-sender retention logic, email size analysis and storage calculation, multi-account sync and action consistency, privacy-focused architecture (no content reading claim verification), rate limiting when performing bulk actions via provider APIs, Free tier action limits and Premium subscription entitlement, and Clean Email account integration.
...
Build an original mobile product inspired by Clean Email's public communication workflow. The V1 clone focuses on email management and cleanup tool with smart views for automatic grouping of emails by sender, type, and age, bulk email cleanup actions (archive, trash, move, label), Auto Clean rules for automatic recurring email management, Unsubscriber for one-tap mailing list removal, Screener for blocking new unknown senders, Keep Newest rule to retain only the latest email from a sender, email size analysis and large email cleanup, read/unread filtering and management, multi-account support (Gmail, Outlook, Yahoo, iCloud, IMAP), privacy-focused design (no reading or selling email content), push notifications for cleanup suggestions, and freemium model with Clean Email Free (limited actions) and Premium subscription.
...
- Preserve the functional job behind Clean Email while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Clean Email branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/clean-email-inbox-cleaner/id1441459725 | iOS listing, privacy labels, release notes, email cleanup claims, and support links | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=com.cleanemail.mobile | Android listing, data safety, email management/cleanup claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official product site | https://clean.email/ | Clean Email inbox cleanup, smart views, and product positioning | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official help | https://clean.email/help | Clean Email app help, setup, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Privacy policy | https://clean.email/privacy | Clean Email data collection, no-content-reading policy, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Terms | https://clean.email/terms | Clean Email services, email management terms, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Clean Email account and email account linking (Gmail OAuth, Outlook OAuth, Yahoo, iCloud, IMAP), smart views email grouping algorithm (sender, type, date, size clustering), bulk action execution at scale (batch archive, trash, move, label across thousands of emails), Auto Clean rule engine (recurring rule scheduling, condition matching, action execution), Unsubscriber mailing list detection and unsubscribe execution (List-Unsubscribe header, web scraping), Screener new sender detection and blocking/approval flow, Keep Newest rule per-sender retention logic, email size analysis and storage calculation, multi-account sync and action consistency, privacy-focused architecture (no content reading claim verification), rate limiting when performing bulk actions via provider APIs, Free tier action limits and Premium subscription entitlement, and Clean Email account integration.

## Core User Journeys

- User creates or restores a Clean Email-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Clean Email.
...
- Manual verification tests for Clean Email account and email account linking (Gmail OAuth, Outlook OAuth, Yahoo, iCloud, IMAP), smart views email grouping algorithm (sender, type, date, size clustering), bulk action execution at scale (batch archive, trash, move, label across thousands of emails), Auto Clean rule engine (recurring rule scheduling, condition matching, action execution), Unsubscriber mailing list detection and unsubscribe execution (List-Unsubscribe header, web scraping), Screener new sender detection and blocking/approval flow, Keep Newest rule per-sender retention logic, email size analysis and storage calculation, multi-account sync and action consistency, privacy-focused architecture (no content reading claim verification), rate limiting when performing bulk actions via provider APIs, Free tier action limits and Premium subscription entitlement, and Clean Email account integration.
...
- Phase 4: Add app-specific safety controls for email management and cleanup tool with smart views for automatic grouping of emails by sender, type, and age, bulk email cleanup actions (archive, trash, move, label), Auto Clean rules for automatic recurring email management, Unsubscriber for one-tap mailing list removal, Screener for blocking new unknown senders, Keep Newest rule to retain only the latest email from a sender, email size analysis and large email cleanup, read/unread filtering and management, multi-account support (Gmail, Outlook, Yahoo, iCloud, IMAP), privacy-focused design (no reading or selling email content), push notifications for cleanup suggestions, and freemium model with Clean Email Free (limited actions) and Premium subscription.
...
- Confirm legal/privacy retention behavior for Clean Email-style sensitive records and update API contracts before downstream implementation.
