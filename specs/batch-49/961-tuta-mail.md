# Tuta Mail-Style Clone Spec

> Metadata
> - Inspiration app: Tuta Mail
...
> - Manual verification blockers: Tuta account creation and secure recovery code generation (no phone number, no backdoor recovery), end-to-end encryption implementation (hybrid RSA/AES, post-quantum encryption roadmap), encrypted calendar zero-knowledge sync and event sharing, encrypted contact book storage and management, custom domain DNS configuration and verification, email alias provisioning and management, full-text encrypted search index (client-side decryption-then-search), password-protected external message delivery (symmetric key exchange via shared secret), two-factor authentication (TOTP, U2F/WebAuthn), Tuta for Teams shared mailbox and user management, whitelabel provisioning for business domains, push notification delivery without leaking content, secure session management and automatic logout, spam filtering on encrypted mailbox, GDPR-compliant data processing and German data center hosting, post-quantum encryption migration roadmap, and Tuta account integration.
...
Build an original mobile product inspired by Tuta Mail's public communication workflow. The V1 clone focuses on end-to-end encrypted email and calendar platform with automatic encryption for Tuta-to-Tuta emails, password-protected messages for external recipients, encrypted calendar with zero-knowledge architecture, encrypted contact book, custom domain support, email aliases, folder and label organization, full-text encrypted search, two-factor authentication (TOTP, U2F), push notifications, secure password recovery without backdoor access, whitelabel for business, Tuta for Teams collaboration, and freemium model with Tuta Free, Revolutionary, Legend, and Business plans.
...
- Preserve the functional job behind Tuta Mail while using original product naming, original UI, original sample data, and licensed integrations.
...
- Do not copy Tuta Mail branding, logos, screenshots, marketing copy, private APIs, proprietary datasets, plan names, UI trade dress, course libraries, textbook solutions, documents, school content, AI prompts, or protected media.
...
|---|---|---|---|
| Apple App Store | https://apps.apple.com/us/app/tuta-mail/id922429609 | iOS listing, privacy labels, release notes, encrypted email claims, and support links | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Google Play | https://play.google.com/store/apps/details?id=de.tutao.tutanota | Android listing, data safety, encrypted email/calendar claims, and release cadence | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official product site | https://tuta.com/ | Tuta encrypted email, calendar, contacts, and product positioning | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Official help | https://tuta.com/support | Tuta app help, encryption setup, and troubleshooting | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Privacy policy | https://tuta.com/privacy-policy | Tuta data collection, encryption architecture, German hosting, and privacy disclosures | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
| Terms | https://tuta.com/terms | Tuta services, email terms, and acceptable-use boundaries | Verified public URL or explicit platform blocker on 2026-05-06; hands-on native behavior still blocked. |
...
- Manual blockers must remain launch-blocking until verified: Tuta account creation and secure recovery code generation (no phone number, no backdoor recovery), end-to-end encryption implementation (hybrid RSA/AES, post-quantum encryption roadmap), encrypted calendar zero-knowledge sync and event sharing, encrypted contact book storage and management, custom domain DNS configuration and verification, email alias provisioning and management, full-text encrypted search index (client-side decryption-then-search), password-protected external message delivery (symmetric key exchange via shared secret), two-factor authentication (TOTP, U2F/WebAuthn), Tuta for Teams shared mailbox and user management, whitelabel provisioning for business domains, push notification delivery without leaking content, secure session management and automatic logout, spam filtering on encrypted mailbox, GDPR-compliant data processing and German data center hosting, post-quantum encryption migration roadmap, and Tuta account integration.

## Core User Journeys

- User creates or restores a Tuta Mail-style account, completes role, school, subscription, or tenant checks, and reaches the dashboard.
...
- Monetization model: use original free/trial/paid/sponsored/school/provider/tenant entitlement logic; do not copy exact pricing, plan names, bundle names, promotional copy, school contracts, tenant offers, or partner offers from Tuta Mail.
...
- Manual verification tests for Tuta account creation and secure recovery code generation (no phone number, no backdoor recovery), end-to-end encryption implementation (hybrid RSA/AES, post-quantum encryption roadmap), encrypted calendar zero-knowledge sync and event sharing, encrypted contact book storage and management, custom domain DNS configuration and verification, email alias provisioning and management, full-text encrypted search index (client-side decryption-then-search), password-protected external message delivery (symmetric key exchange via shared secret), two-factor authentication (TOTP, U2F/WebAuthn), Tuta for Teams shared mailbox and user management, whitelabel provisioning for business domains, push notification delivery without leaking content, secure session management and automatic logout, spam filtering on encrypted mailbox, GDPR-compliant data processing and German data center hosting, post-quantum encryption migration roadmap, and Tuta account integration.
...
- Phase 4: Add app-specific safety controls for end-to-end encrypted email and calendar platform with automatic encryption for Tuta-to-Tuta emails, password-protected messages for external recipients, encrypted calendar with zero-knowledge architecture, encrypted contact book, custom domain support, email aliases, folder and label organization, full-text encrypted search, two-factor authentication (TOTP, U2F), push notifications, secure password recovery without backdoor access, whitelabel for business, Tuta for Teams collaboration, and freemium model with Tuta Free, Revolutionary, Legend, and Business plans.
...
- Confirm legal/privacy retention behavior for Tuta Mail-style sensitive records and update API contracts before downstream implementation.
