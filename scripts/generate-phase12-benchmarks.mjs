...
const OUT_DIR = join(ROOT, "tasks", "scorecards", "phase-12");
const VALIDATION_SUMMARY = join(OUT_DIR, "validation-summary.json");
const RUN_ID = "phase-12-local-2026-05-14";
const CATEGORY = "Social, Dating & Community";
...
for (const result of validation.results) {
  ...
  for (const variant of VARIANTS) {
    ...
    if (!validationCommand) {
      blockers.push(blocker(result, variant, "missing-validation-evidence", `No Step 12.11 validation command found for ${variant}.`));
      continue;
    }

    if (validationCommand.status === "blocker") {
      const reason = variant === "flutter" ? "missing-local-flutter-toolchain" : variant === "android-native" ? "missing-local-android-toolchain" : "validation-blocked";
      blockers.push(blocker(result, variant, reason, validationCommand.detail));
      continue;
    }
    ...
    if (!BENCHMARKABLE_VARIANTS.has(variant)) {
      blockers.push(blocker(result, variant, "unsupported-harness-variant", `${variant} is not enabled for Phase 12 local benchmarking.`));
      continue;
    }
...
await writeFile(join(OUT_DIR, "README.md"), `# Phase 12 Benchmark Scorecards
